...
function createReplyConfig(home: string, streamMode?: "block"): GensparxConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("gensparx-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("GENSPARX_TEST_FAST", "1");
