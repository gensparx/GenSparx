...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop gensparx!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@gensparx_bot", { botUsername: "gensparx_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@gensparx_bot", { botUsername: "gensparx_bot" })).toBe(true);
