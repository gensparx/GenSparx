...
import { scheduleGatewaySigusr1Restart, triggerGensparxRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting gensparx in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerGensparxRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting gensparx via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
