...
export function matchesMentionPatterns(text: string, mentionRegexes: RegExp[]): boolean {
  ...
  const legacyBrand = cleaned.replace(/\bgensparx\b/g, "openclaw");
  return mentionRegexes.some((re) => re.test(cleaned) || re.test(legacyBrand));
}
...
export function matchesMentionWithExplicit(params: {
  ...
  const cleanedLegacy = cleaned.replace(/\bgensparx\b/g, "openclaw");
  const padded = ` ${cleaned}`;
  const paddedLegacy = ` ${cleanedLegacy}`;
  ...
  if (hasAnyMention && explicitAvailable) {
    return (
      explicit ||
      params.mentionRegexes.some(
        (re) =>
          re.test(cleaned) ||
          re.test(padded) ||
          re.test(cleanedLegacy) ||
          re.test(paddedLegacy),
      )
    );
  }
  if (!textToCheck) {
  ...
  return (
    ...
    params.mentionRegexes.some(
      (re) =>
        re.test(cleaned) || re.test(padded) || re.test(cleanedLegacy) || re.test(paddedLegacy),
    )
  );
}
