...
type HomeEnvSnapshot = {
  ...
  GENSPARX_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    GENSPARX_AGENT_DIR: process.env.GENSPARX_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.GENSPARX_AGENT_DIR = path.join(home, ".gensparx", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".gensparx", "agent");
