...
export function isCommandEnabled(cfg: GensparxConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: GensparxConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as GensparxConfig),
...
export function getCommandDetection(_cfg?: GensparxConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: GensparxConfig) {
