...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    gensparxHome: process.env.GENSPARX_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("GENSPARX_HOME", snapshot.gensparxHome);
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.GENSPARX_STATE_DIR = join(home, ".gensparx");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "gensparx-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".gensparx", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): GensparxConfig {
  return {
    agents: {
      defaults: {
