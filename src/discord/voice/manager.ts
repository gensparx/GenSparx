...
function resolveVoiceTtsConfig(params: { cfg: GensparxConfig; override?: TtsConfig }): {
  cfg: GensparxConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredGensparxTmpDir(), "discord-voice-"));
