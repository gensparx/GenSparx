...
type HomeEnvSnapshot = {
  ...
  GENSPARX_HOME: string | undefined;
  GENSPARX_STATE_DIR: string | undefined;
};
...
function restoreHomeEnv(snapshot: HomeEnvSnapshot) {
  ...
  restoreValue("GENSPARX_HOME");
  restoreValue("GENSPARX_STATE_DIR");
}

async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const home = path.join(suiteTempHomeRoot, `case-${suiteTempHomeCaseId++}`);
  await fs.mkdir(path.join(home, ".gensparx", "agents", "main", "sessions"), { recursive: true });
...
describe("runCronIsolatedAgentTurn", () => {
  beforeAll(async () => {
    suiteTempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-cron-turn-suite-"));
  });
  ...
  it("passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.agentDir).toBe(path.join(home, ".gensparx", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".gensparx", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
