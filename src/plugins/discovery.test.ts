...
import { clearPluginDiscoveryCache, discoverGensparxPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `gensparx-plugins-${randomUUID()}`);
  ...
  return await withEnvAsync(
    {
      ...
      GENSPARX_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverGensparxPlugins>[0],
) {
  return await withStateDir(stateDir, async () => {
    return discoverGensparxPlugins(params);
  });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      gensparx: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverGensparxPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".gensparx", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverGensparxPlugins({ workspaceDir });
    });
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverGensparxPlugins({});
    });
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@gensparx/voice-call",
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@gensparx/demo-plugin-dir",
      extensions: ["./index.js"],
    });
    fs.writeFileSync(path.join(packDir, "index.js"), "module.exports = {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverGensparxPlugins({ extraPaths: [packDir] });
    });
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@gensparx/escape-pack",
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverGensparxPlugins({ ownershipUid: actualUid + 1 });
      });
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = await withEnvAsync(
      {
        GENSPARX_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
      async () => withStateDir(stateDir, async () => discoverGensparxPlugins({})),
    );
