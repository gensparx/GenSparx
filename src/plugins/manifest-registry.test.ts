...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `gensparx-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "gensparx.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "gensparx.plugin.json");
  const linkedManifest = path.join(rootDir, "gensparx.plugin.json");
