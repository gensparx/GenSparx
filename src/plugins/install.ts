...
async function ensureOpenClawExtensions(manifest: PackageManifest) {
  const extensions =
    manifest[MANIFEST_KEY]?.extensions ??
    LEGACY_MANIFEST_KEYS.map((key) => manifest[key]?.extensions).find(Array.isArray);
  if (!Array.isArray(extensions)) {
    throw new Error(
      `package.json missing ${MANIFEST_KEY}.extensions or ${LEGACY_MANIFEST_KEYS[0]}.extensions`,
    );
  }
