...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_GENSPARX_EXTENSIONS: "missing_gensparx_extensions",
  EMPTY_GENSPARX_EXTENSIONS: "empty_gensparx_extensions",
...
function ensureGensparxExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_GENSPARX_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json gensparx.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_GENSPARX_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureGensparxExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from gensparx.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-gensparx"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "gensparx security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "gensparx security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "gensparx-npm-pack-",
    spec,
    timeoutMs,
