...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const entry of pluginSdkScopedAliasEntries) {
    ...
    if (resolved) {
      aliasMap[`gensparx/plugin-sdk/${entry.subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: GensparxPluginDefinition;
  register?: GensparxPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as GensparxPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as GensparxPluginDefinition;
...
export function loadGensparxPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "gensparx/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: GensparxPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as GensparxPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
