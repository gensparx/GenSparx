...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".gensparx");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.GENSPARX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$GENSPARX_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via GENSPARX_STATE_DIR.
