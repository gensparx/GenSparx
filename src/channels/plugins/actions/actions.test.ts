...
function telegramCfg(): GensparxConfig {
  return { channels: { telegram: { botToken: "tok" } } } as GensparxConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: GensparxConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as GensparxConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as GensparxConfig;
...
function createSignalAccountOverrideCfg(): GensparxConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as GensparxConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as GensparxConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as GensparxConfig,
        accountId: undefined,
        params: {
