...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set GENSPARX_GATEWAY_TOKEN/GENSPARX_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "gensparx-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.GENSPARX_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.GENSPARX_GATEWAY_PASSWORD);
