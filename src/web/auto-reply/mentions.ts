...
export function isBotMentionedFromTargets(
  ...
  const bodyPadded = ` ${bodyClean}`;
  // Backward-compatibility: accept both "gensparx" and legacy "openclaw" mention regexes.
  const bodyLegacyBrand = bodyClean.replace(/\bgensparx\b/g, "openclaw");
  const bodyLegacyBrandPadded = ` ${bodyLegacyBrand}`;
  if (
    mentionCfg.mentionRegexes.some(
      (re) =>
        re.test(bodyClean) ||
        re.test(bodyPadded) ||
        re.test(bodyLegacyBrand) ||
        re.test(bodyLegacyBrandPadded),
    )
  ) {
    return true;
  }
...
export function debugMention(
  ...
  const details = {
    from: msg.from,
    body: msg.body,
    bodyClean: bodyClean.replace(/\bgensparx\b/g, "openclaw"),
