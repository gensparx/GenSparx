  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.expectPairingText) {
        ...
        expect(pairingText, testCase.name).toContain("pairing approve telegram");
        expect(pairingText, testCase.name).toMatch(/(?:gensparx|openclaw) pairing approve telegram/i);
      }
    }
  });
