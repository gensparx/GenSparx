...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".gensparx");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      GENSPARX_STATE_DIR: paths.stateDir,
      GENSPARX_CONFIG_PATH: paths.configPath,
