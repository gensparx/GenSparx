...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    GENSPARX_CONFIG_DIR: join(sandbox.rootDir, "config"),
    GENSPARX_WORKSPACE_DIR: join(sandbox.rootDir, "gensparx"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENSPARX_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      GENSPARX_EXTRA_MOUNTS: undefined,
      GENSPARX_HOME_VOLUME: "gensparx-home",
    });
    ...
    expect(envFile).toContain("GENSPARX_HOME_VOLUME=gensparx-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("gensparx-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("gensparx-home:");
    ...
    expect(log).toContain("run --rm gensparx-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm gensparx-cli config set gateway.mode local");
    expect(log).toContain("run --rm gensparx-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENSPARX_CONFIG_DIR: configDir,
      GENSPARX_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when GENSPARX_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("GENSPARX_GATEWAY_TOKEN=config-token-123");
  });

  it("treats GENSPARX_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("GENSPARX_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg GENSPARX_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg GENSPARX_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      GENSPARX_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker gensparx-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        GENSPARX_SANDBOX: "1",
        GENSPARX_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("gensparx-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps gensparx-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate gensparx-gateway"));
  ...
  it("rejects injected multiline GENSPARX_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENSPARX_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("GENSPARX_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid GENSPARX_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENSPARX_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid GENSPARX_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      GENSPARX_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("GENSPARX_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:gensparx-gateway"');
