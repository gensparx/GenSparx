...
const TEMP_MEDIA_PREFIX = "gensparx-media-";
...
function createGroqAudioConfig(): GensparxConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): GensparxConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): GensparxConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: GensparxConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredGensparxTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        GENSPARX_AGENT_DIR: isolatedAgentDir,
