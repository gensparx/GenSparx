...
import type { ConfigFileSnapshot } from "./types.gensparx.js";
import { GensparxSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(GensparxSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.gensparx/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/gensparx/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/gensparx/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/gensparx/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/gensparx/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = GensparxSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "GensparxConfig";
    const hints = mainSchemaHints;
