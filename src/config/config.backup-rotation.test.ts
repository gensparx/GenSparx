...
describe("config backup rotation", () => {
  it("keeps a 5-deep backup ring for config writes", async () => {
    await withTempHome(async () => {
      const stateDir =
        process.env.OPENCLAW_STATE_DIR?.trim() || process.env.GENSPARX_STATE_DIR?.trim();
      if (!stateDir) {
        throw new Error("Expected OPENCLAW_STATE_DIR or GENSPARX_STATE_DIR to be set by withTempHome");
      }
