...
import type { GensparxConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareGensparxVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  gensparx config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  gensparx config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: GensparxConfig,
  pathSegments: string[],
): { changed: boolean; next: GensparxConfig } {
...
function coerceConfig(value: unknown): GensparxConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as GensparxConfig;
}
...
function stampConfigVersion(cfg: GensparxConfig): GensparxConfig {
...
function warnIfConfigFromFuture(cfg: GensparxConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareGensparxVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as GensparxConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): GensparxConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as GensparxConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: GensparxConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as GensparxConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.GENSPARX_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.GENSPARX_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.GENSPARX_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.GENSPARX_WATCH_SESSION === "string" &&
        deps.env.GENSPARX_WATCH_SESSION.trim().length > 0
          ? deps.env.GENSPARX_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.GENSPARX_WATCH_COMMAND === "string" &&
        deps.env.GENSPARX_WATCH_COMMAND.trim().length > 0
          ? deps.env.GENSPARX_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `GENSPARX_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: GensparxConfig | null = null;
let runtimeConfigSourceSnapshot: GensparxConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GENSPARX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.GENSPARX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): GensparxConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): GensparxConfig {
