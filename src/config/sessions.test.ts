...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ GENSPARX_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-gensparx",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-gensparx#general");
  });
  ...
  it("derives session transcripts dir from GENSPARX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { GENSPARX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when GENSPARX_STATE_DIR differs from stored paths", () => {
