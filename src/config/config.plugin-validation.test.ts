...
describe("config plugin validation", () => {
  ...
  it("warns for removed legacy plugin ids instead of failing validation", async () => {
    ...
    expect(res.ok).toBe(true);
    if (res.ok) {
      // Some forks still ship this plugin; validation must remain non-fatal either way.
      const removedWarnings = res.warnings.filter((warning) =>
        warning.message.includes("plugin removed: google-antigravity-auth"),
      );
      expect(removedWarnings.length === 0 || removedWarnings.length === 4).toBe(true);
    }
  });
