...
import type { GensparxConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: GensparxConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: GensparxConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: GensparxConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = GensparxSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as GensparxConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as GensparxConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as GensparxConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as GensparxConfig,
  };
}
