...
function isWhatsAppConfigured(cfg: GensparxConfig): boolean {
...
function isGenericChannelConfigured(cfg: GensparxConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: GensparxConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: GensparxConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: GensparxConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: GensparxConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: GensparxConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: GensparxConfig, pluginId: string): GensparxConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
