...
import { buildWebSearchProviderConfig, withTempHome, writeGensparxConfig } from "./test-helpers.js";
import { GensparxSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = GensparxSchema.safeParse({
      $schema: "https://gensparx.com/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://gensparx.com/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = GensparxSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = GensparxSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeGensparxConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeGensparxConfig(home, {
        gateway: { bind: "${GENSPARX_BIND}" },
      });

      const prev = process.env.GENSPARX_BIND;
      process.env.GENSPARX_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.GENSPARX_BIND;
        } else {
          process.env.GENSPARX_BIND = prev;
        }
      }
    });
  });
