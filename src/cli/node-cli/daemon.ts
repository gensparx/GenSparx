...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("gensparx node install"), formatCliCommand("gensparx node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("gensparx node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    GENSPARX_LOG_PREFIX: baseEnv.GENSPARX_LOG_PREFIX ?? "node",
