    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "gensparx node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["gensparx node status", "Check node host service status."],
          ["gensparx node install", "Install the node host service."],
          ["gensparx node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.gensparx.com/cli/node")}\n`,
    );
