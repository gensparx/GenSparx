...
import type { GensparxConfig, ConfigFileSnapshot } from "../config/types.gensparx.js";
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/gensparx-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as GensparxConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/gensparx-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveGensparxPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("gensparx-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    readPackageName.mockResolvedValue("gensparx");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/gensparx-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "gensparx-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as GensparxConfig,
        });
      },
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("gensparx-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ GENSPARX_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.GENSPARX_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("gensparx-update-wizard");
    await withEnvAsync({ GENSPARX_GIT_DIR: tempDir }, async () => {
      setTty(true);
