...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@gensparx/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@gensparx/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@gensparx/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@gensparx/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @gensparx/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@gensparx/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@gensparx/plugin-alpha@1.2.3",
        installPath: "/tmp/gensparx/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@gensparx/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @gensparx/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@gensparx/plugin-alpha@latest",
      true,
      "/tmp/gensparx/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@gensparx/plugin-alpha@latest",
      installPath: "/tmp/gensparx/extensions/alpha",
      version: "1.2.3",
