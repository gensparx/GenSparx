...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.gensparx${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.GENSPARX_PROFILE = profile;

  const stateDir = env.GENSPARX_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  ...
  if (profile === "dev" && !env.GENSPARX_GATEWAY_PORT?.trim()) {
