...
function resolveProfileStateDir(
  ...
  const hasOpenClawHome = Boolean(env.OPENCLAW_HOME?.trim());
  const home = hasOpenClawHome
    ? resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir)
    : homedir();
  const prefix = hasOpenClawHome ? ".openclaw" : ".gensparx";
  return path.join(home, `${prefix}${suffix}`);
}
