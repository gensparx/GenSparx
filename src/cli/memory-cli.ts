    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["gensparx memory status", "Show index and provider status."],
          ["gensparx memory index --force", "Force a full reindex."],
          ['gensparx memory search --query "deployment notes"', "Search indexed memory entries."],
          ["gensparx memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.gensparx.com/cli/memory")}\n`,
    );
