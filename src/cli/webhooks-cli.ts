...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.gensparx.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + GenSparx hooks")
    ...
    .option("--hook-url <url>", "GenSparx hook URL")
    .option("--hook-token <token>", "GenSparx hook token")
