...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["GENSPARX_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-bundled-skills-test-"));
    process.env.GENSPARX_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
