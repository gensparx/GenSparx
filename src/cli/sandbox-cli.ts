...
const SANDBOX_EXAMPLES = {
  main: [
    ["gensparx sandbox list", "List all sandbox containers."],
    ["gensparx sandbox list --browser", "List only browser containers."],
    ["gensparx sandbox recreate --all", "Recreate all containers."],
    ["gensparx sandbox recreate --session main", "Recreate a specific session."],
    ["gensparx sandbox recreate --agent mybot", "Recreate agent containers."],
    ["gensparx sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["gensparx sandbox list", "List all sandbox containers."],
    ["gensparx sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["gensparx sandbox recreate --all", "Recreate all containers."],
    ["gensparx sandbox recreate --session main", "Recreate a specific session."],
    ["gensparx sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["gensparx sandbox recreate --browser --all", "Recreate only browser containers."],
    ["gensparx sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["gensparx sandbox explain", "Show effective sandbox config."],
    ["gensparx sandbox explain --session agent:main:main", "Explain a specific session."],
    ["gensparx sandbox explain --agent work", "Explain an agent sandbox."],
    ["gensparx sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.gensparx.com/cli/sandbox")}\n`,
    )
    .action(() => {
