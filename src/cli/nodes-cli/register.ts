    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["gensparx nodes status", "List known nodes with live status."],
          ["gensparx nodes pairing pending", "Show pending node pairing requests."],
          ['gensparx nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["gensparx nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.gensparx.com/cli/nodes")}\n`,
    );
