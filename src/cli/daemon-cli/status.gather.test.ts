...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    GENSPARX_STATE_DIR: "/tmp/gensparx-daemon",
    GENSPARX_CONFIG_PATH: "/tmp/gensparx-daemon/gensparx.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.GENSPARX_STATE_DIR ?? "/tmp/gensparx-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.GENSPARX_CONFIG_PATH ?? `${stateDir}/gensparx.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/gensparx-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.GENSPARX_STATE_DIR = "/tmp/gensparx-cli";
    process.env.GENSPARX_CONFIG_PATH = "/tmp/gensparx-cli/gensparx.json";
