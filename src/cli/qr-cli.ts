...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = typeof env.GENSPARX_GATEWAY_TOKEN === "string" ? env.GENSPARX_GATEWAY_TOKEN : "";
...
function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.GENSPARX_GATEWAY_PASSWORD === "string" ? env.GENSPARX_GATEWAY_PASSWORD : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy =
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.gensparx.com/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the GenSparx iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("gensparx devices list")}`,
          `  ${theme.command("gensparx devices approve <requestId>")}`,
        );
