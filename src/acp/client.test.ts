...
const createTempDir = () => tempDirs.make("gensparx-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets GENSPARX_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "gensparx",
    });

    expect(env.GENSPARX_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("gensparx");
  });

  it("overrides pre-existing GENSPARX_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      GENSPARX_SHELL: "wrong",
    });
    expect(env.GENSPARX_SHELL).toBe("acp-client");
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "gensparx", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "gensparx", "dist", "entry.js");
    const shimPath = path.join(dir, "gensparx.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\gensparx\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/gensparx-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/gensparx-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/gensparx-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
