...
export function loadShellEnvFallback(opts: ShellEnvFallbackOptions): ShellEnvFallbackResult {
  ...
  if (!probe.ok) {
    logger.warn(`[gensparx] shell env fallback failed: ${probe.error}`);
    lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.GENSPARX_LOAD_SHELL_ENV) || isTruthyEnvValue(env.OPENCLAW_LOAD_SHELL_ENV)
  );
}

export function shouldDeferShellEnvFallback(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.GENSPARX_DEFER_SHELL_ENV_FALLBACK) ||
    isTruthyEnvValue(env.OPENCLAW_DEFER_SHELL_ENV_FALLBACK)
  );
}

export function resolveShellEnvFallbackTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw =
    env.GENSPARX_SHELL_ENV_TIMEOUT_MS?.trim() ?? env.OPENCLAW_SHELL_ENV_TIMEOUT_MS?.trim();
  if (!raw) {
    return DEFAULT_TIMEOUT_MS;
  }
