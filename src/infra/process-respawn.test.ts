...
const triggerGensparxRestartMock = vi.hoisted(() => vi.fn());

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./restart.js", () => ({
  triggerGensparxRestart: (...args: unknown[]) => triggerGensparxRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerGensparxRestartMock.mockClear();
  if (originalPlatformDescriptor) {
...
function expectLaunchdKickstartSupervised(params?: { launchJobLabel?: string }) {
  ...
  expect(triggerGensparxRestartMock).toHaveBeenCalledOnce();
  expect(spawnMock).not.toHaveBeenCalled();
}

describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when GENSPARX_NO_RESPAWN is set", () => {
    process.env.GENSPARX_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
  ...
  it("runs launchd kickstart helper on macOS when launchd label is set", () => {
    expectLaunchdKickstartSupervised({ launchJobLabel: "ai.gensparx.gateway" });
  });
  ...
  it("returns failed when launchd kickstart helper fails", () => {
    ...
    process.env.LAUNCH_JOB_LABEL = "ai.gensparx.gateway";
    process.env.GENSPARX_LAUNCHD_LABEL = "ai.gensparx.gateway";
    triggerGensparxRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
      detail: "spawn failed",
    });
  ...
  it("returns supervised when GENSPARX_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
  ...
  it("returns supervised when GENSPARX_SERVICE_MARKER is set", () => {
    clearSupervisorHints();
    process.env.GENSPARX_SERVICE_MARKER = "gateway";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
