...
import { resolveGensparxPackageRoot, resolveGensparxPackageRootSync } from "./gensparx-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveGensparxPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "gensparx" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveGensparxPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // gensparx.mjs or dist/<bundle>.js
