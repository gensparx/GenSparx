...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses gensparx-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "gensparx-gateway" },
          { pid: process.pid, cmd: "gensparx-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'gensparx'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple gensparx pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "gensparx-gateway" },
          { pid: pid2, cmd: "gensparx-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-gensparx pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks gensparx processes — non-gensparx listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "gensparx-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty gensparx stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty gensparx stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + gensparx pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "gensparx-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'gensparx'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("gensparx")
      // inside the mid-loop flush: a non-gensparx cmd between two entries must not
      // be pushed, but the following gensparx entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-gensparx entry first, then gensparx entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid gensparx entry must still be found.
      const stdout = `p0\ncgensparx-gateway\np${stalePid}\ncgensparx-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero gensparx pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-gensparx stdout (line 145)", () => {
    it("treats status 1 + non-gensparx stdout as port-free (not an gensparx process)", () => {
      // status 1 + non-empty stdout where no gensparx pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill gensparx pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-gensparx output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no gensparx pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
