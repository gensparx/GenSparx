...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["GENSPARX_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    if (!loaded) {
      ({ resolveGensparxPackageRoot } = await import("./gensparx-root.js"));
    ...
    vi.mocked(resolveGensparxPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveGensparxPackageRoot).mockResolvedValue("/opt/gensparx");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/gensparx/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
