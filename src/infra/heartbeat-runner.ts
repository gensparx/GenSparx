...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: GensparxConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: GensparxConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: GensparxConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: GensparxConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: GensparxConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: GensparxConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: GensparxConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
