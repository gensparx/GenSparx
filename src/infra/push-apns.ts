...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.GENSPARX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.GENSPARX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set GENSPARX_APNS_TEAM_ID and GENSPARX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.GENSPARX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.GENSPARX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.GENSPARX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set GENSPARX_APNS_PRIVATE_KEY_P8 or GENSPARX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading GENSPARX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createGensparxPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    gensparx: createGensparxPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
