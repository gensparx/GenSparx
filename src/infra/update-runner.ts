...
const DEFAULT_PACKAGE_NAME = "gensparx";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["gensparx", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "gensparx.mjs");
    ...
    const doctorStep = await runStep(
      step("gensparx doctor", doctorArgv, gitRoot, { GENSPARX_UPDATE_IN_PROGRESS: "1" }),
    );
