...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: GensparxConfig | null = null;
  ...
  get(key: string, cfg: GensparxConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: GensparxConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: GensparxConfig): void {
