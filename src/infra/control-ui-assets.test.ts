...
vi.mock("./gensparx-root.js", () => ({
  resolveGensparxPackageRoot: vi.fn(async () => null),
  resolveGensparxPackageRootSync: vi.fn(() => null),
}));
...
let gensparxRoot: typeof import("./gensparx-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    gensparxRoot = await import("./gensparx-root.js");
  });
  ...
  it("uses resolveGensparxPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/gensparx");
    (
      gensparxRoot.resolveGensparxPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/gensparx"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "gensparx" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "gensparx.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-gensparx");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/gensparx-bundle");
    (
      gensparxRoot.resolveGensparxPackageRootSync as unknown as ReturnType<typeof vi.fn>
