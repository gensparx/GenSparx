...
export function triggerGensparxRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.GENSPARX_SYSTEMD_UNIT,
        process.env.GENSPARX_PROFILE,
      );
  ...
  const label =
    process.env.GENSPARX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.GENSPARX_PROFILE);
