...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-update-"));
  });
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "gensparx", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm gensparx doctor --non-interactive": { stdout: "" },
    });
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g gensparx@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
    ...
    expect(calls.some((call) => call === "npm i -g gensparx@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g gensparx@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g gensparx@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "gensparx", version: "2.0.0" }),
            "utf-8",
