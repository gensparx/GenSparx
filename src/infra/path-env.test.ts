...
let ensureGensparxCliOnPath: typeof import("./path-env.js").ensureGensparxCliOnPath;

describe("ensureGensparxCliOnPath", () => {
  const envKeys = [
    "PATH",
    "GENSPARX_PATH_BOOTSTRAPPED",
    "GENSPARX_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureGensparxCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling gensparx exists", () => {
    const tmp = abs("/tmp/gensparx-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "gensparx");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/gensparx-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "gensparx");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/gensparx-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "gensparx");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "gensparx");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/gensparx-path/case-linuxbrew");
