...
describe("ensureGenSparxCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling gensparx exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-path-"));
    try {
      ...
      const originalFlag = process.env.GENSPARX_PATH_BOOTSTRAPPED;
