...
function candidateBinDirs(opts: EnsureGensparxPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `gensparx` lives next to the executable (process.execPath).
  try {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.GENSPARX_ALLOW_PROJECT_LOCAL_BIN);
...
/**
 * Best-effort PATH bootstrap so skills that require the `gensparx` CLI can run
