...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.GENSPARX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*gensparx-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "gensparx";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "gensparx");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "gensparx-gw");
  ...
  records.push(`_gensparx-gw._tcp IN PTR ${instanceLabel}._gensparx-gw._tcp`);
  records.push(`${instanceLabel}._gensparx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._gensparx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; gensparx-content-hash: ${contentHash}\n${contentBody}`;
}
