...
describe("resolveEffectiveHomeDir", () => {
  it("prefers GENSPARX_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/gensparx-home"),
    );
  });
  ...
  it("expands GENSPARX_HOME when set to ~", () => {
    const env = {
      GENSPARX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for GENSPARX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { GENSPARX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when GENSPARX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ GENSPARX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { GENSPARX_HOME: "/srv/gensparx-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/gensparx-home")}/x`);
  });
