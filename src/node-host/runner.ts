...
function ensureNodePathEnv(): string {
  ensureGensparxCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.GENSPARX_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.GENSPARX_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
