...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "gensparx-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-lid-b-"));
...
describe("resolveUserPath", () => {
  it("expands ~ to home dir", () => {
    expect(resolveUserPath("~")).toBe(path.resolve(resolveHomeDir() ?? os.homedir()));
  });

  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/gensparx")).toBe(
      path.resolve(resolveHomeDir() ?? os.homedir(), "gensparx"),
    );
  });
