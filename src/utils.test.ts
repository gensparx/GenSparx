...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("gensparx-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("gensparx-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("gensparx-lid-a-", (first) => {
      withTempDirSync("gensparx-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.gensparx when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "gensparx-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers GENSPARX_HOME over HOME", () => {
    vi.stubEnv("GENSPARX_HOME", "/srv/gensparx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/gensparx-home"));
...
describe("shortenHomePath", () => {
  it("uses $GENSPARX_HOME prefix when GENSPARX_HOME is set", () => {
    vi.stubEnv("GENSPARX_HOME", "/srv/gensparx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/gensparx-home")}/.gensparx/gensparx.json`)).toBe(
      "$GENSPARX_HOME/.gensparx/gensparx.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $GENSPARX_HOME replacement when GENSPARX_HOME is set", () => {
    vi.stubEnv("GENSPARX_HOME", "/srv/gensparx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/gensparx-home")}/.gensparx/gensparx.json`),
    ).toBe("config: $GENSPARX_HOME/.gensparx/gensparx.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/gensparx")).toBe(path.resolve(os.homedir(), "gensparx"));
  });
  ...
  it("prefers GENSPARX_HOME for tilde expansion", () => {
    vi.stubEnv("GENSPARX_HOME", "/srv/gensparx-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/gensparx")).toBe(path.resolve("/srv/gensparx-home", "gensparx"));
