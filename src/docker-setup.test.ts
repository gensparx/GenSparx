...
function runDockerSetup(
  ...
  if (process.platform === "win32") {
    const wslRoot = toWslPath(sandbox.rootDir);
    const envPairs = Object.entries(env).map(([key, rawValue]) => {
      const value = rawValue ?? "";
      if (key === "PATH") {
        const converted = value
          .split(delimiter)
          .filter(Boolean)
          .map((entry) => toWslPath(entry))
          .join(":");
        return `${key}=${shellQuote(`/usr/bin:/bin:${converted}`)}`;
      }
      if (
        key === "DOCKER_STUB_LOG" ||
        key === "HOME" ||
        ...
        return `${key}=${shellQuote(toWslPath(value))}`;
      }
      return `${key}=${shellQuote(value)}`;
    });
    return spawnSync("bash", ["-lc", `cd ${shellQuote(wslRoot)} && ${envPairs.join(" ")} ./docker-setup.sh`], {
      env: process.env,
      encoding: "utf8",
  ...
  return spawnSync("bash", ["-c", "./docker-setup.sh"], {
    cwd: sandbox.rootDir,
    env,
    encoding: "utf8",
    stdio: ["ignore", "ignore", "pipe"],
