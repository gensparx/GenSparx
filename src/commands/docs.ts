...
const SEARCH_TOOL = "https://docs.gensparx.com/mcp.SearchGensparx";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.gensparx.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('gensparx docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.gensparx.com/");
      runtime.log(`Search: ${formatCliCommand('gensparx docs "your query"')}`);
    }
    return;
  }
