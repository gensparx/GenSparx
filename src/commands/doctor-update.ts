...
async function detectGenSparxGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
  ...
  return res.stdout.trim() === root ? "git" : "git";
}
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress =
    isTruthyEnvValue(process.env.GENSPARX_UPDATE_IN_PROGRESS) ||
  ...
  const git = await detectGenSparxGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update GenSparx from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("gensparx update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
