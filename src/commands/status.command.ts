...
export async function statusCommand(
  ...
  runtime.log(theme.heading("GenSparx status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("gensparx security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("gensparx security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.gensparx.com/faq");
  runtime.log("Troubleshooting: https://docs.gensparx.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("gensparx status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("gensparx logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("gensparx status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("gensparx gateway probe")}`);
  }
}
