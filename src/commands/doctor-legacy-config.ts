...
export function normalizeCompatibilityConfigValues(cfg: GensparxConfig): {
  config: GensparxConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: GensparxConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as GensparxConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as GensparxConfig["browser"],
    };
    changes.push(
