...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("GenSparx status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("gensparx status --all") ||
          l.includes("gensparx --profile isolated status --all") ||
          l.includes("gensparx status --all") ||
          l.includes("gensparx --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.GENSPARX_GATEWAY_TOKEN = "abcd1234";
    try {
