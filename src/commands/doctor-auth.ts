...
function pruneAuthProfiles(
  cfg: GensparxConfig,
  profileIds: Set<string>,
): { next: GensparxConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("gensparx models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("gensparx models auth setup-token")} or ${formatCliCommand(
      "gensparx configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "gensparx models auth login --provider openai-codex",
    )} or ${formatCliCommand("gensparx configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("gensparx configure")}\` or \`${formatCliCommand("gensparx onboard")}\`.`;
}
