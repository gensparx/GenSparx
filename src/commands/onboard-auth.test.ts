...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under GENSPARX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GENSPARX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-minimax-"));
    process.env.GENSPARX_STATE_DIR = tempStateDir;
    process.env.GENSPARX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.GENSPARX_AGENT_DIR;
