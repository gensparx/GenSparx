...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under GENSPARX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("gensparx-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-oauth-sync-"));
    ...
    process.env.GENSPARX_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-oauth-nosync-"));
    process.env.GENSPARX_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside GENSPARX_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-oauth-external-"));
    process.env.GENSPARX_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* GENSPARX_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GENSPARX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("gensparx-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
