  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("gensparx config set agents.defaults.memorySearch.provider openai")}`,
          "",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("gensparx configure --section model")}`,
        `- To disable: ${formatCliCommand("gensparx config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("gensparx memory status --deep")}`,
      ].join("\n"),
      "Memory search",
