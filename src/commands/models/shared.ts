...
export async function loadValidConfigOrThrow(): Promise<GensparxConfig> {
...
export async function updateConfig(
  mutator: (cfg: GensparxConfig) => GensparxConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: GensparxConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: GensparxConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("gensparx agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
