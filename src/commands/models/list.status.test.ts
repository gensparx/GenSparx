...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveGensparxAgentDir: vi.fn().mockReturnValue("/tmp/gensparx-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/gensparx-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/gensparx-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveGensparxAgentDir: mocks.resolveGensparxAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/gensparx-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveGensparxAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/gensparx-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/gensparx-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/gensparx-agent-custom");
