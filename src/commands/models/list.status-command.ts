...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveGenSparxAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("gensparx models auth setup-token")}\` or \`${formatCliCommand("gensparx configure")}\`.`
          : `Run \`${formatCliCommand("gensparx configure")}\` or set an API key env var.`;
