...
function resolveAgentName(cfg: GensparxConfig, agentId: string) {
...
function resolveAgentModel(cfg: GensparxConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: GensparxConfig): AgentSummary[] {
