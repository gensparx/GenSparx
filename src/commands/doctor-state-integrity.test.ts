...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("prints CLI verification hints when recent sessions are missing transcripts", async () => {
    ...
    expect(text).toMatch(/(?:openclaw|gensparx) sessions --store ".*sessions\.json"/);
    expect(text).toMatch(
      /(?:openclaw|gensparx) sessions cleanup --store ".*sessions\.json" --dry-run/,
    );
    expect(text).toMatch(
      /(?:openclaw|gensparx) sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
