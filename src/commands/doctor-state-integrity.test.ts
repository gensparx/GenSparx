...
type EnvSnapshot = {
  HOME?: string;
  GENSPARX_HOME?: string;
  GENSPARX_STATE_DIR?: string;
  GENSPARX_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    GENSPARX_HOME: process.env.GENSPARX_HOME,
    GENSPARX_STATE_DIR: process.env.GENSPARX_STATE_DIR,
    GENSPARX_OAUTH_DIR: process.env.GENSPARX_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: GensparxConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: GensparxConfig) {
...
async function runStateIntegrityText(cfg: GensparxConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.GENSPARX_HOME = tempHome;
    process.env.GENSPARX_STATE_DIR = path.join(tempHome, ".gensparx");
    delete process.env.GENSPARX_OAUTH_DIR;
    fs.mkdirSync(process.env.GENSPARX_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when GENSPARX_OAUTH_DIR is explicitly configured", async () => {
    process.env.GENSPARX_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: GensparxConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints gensparx-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: GensparxConfig = {};
    ...
    expect(text).toMatch(/gensparx sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/gensparx sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /gensparx sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
