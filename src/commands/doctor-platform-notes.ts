...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`GENSPARX_${key.slice(key.indexOf("_") + 1)}\` instead (OPENCLAW_* is still accepted).`,
      ),
    ];
  ...
  const passwordEntries =
    cfg.gateway?.auth?.password?.trim() !== undefined
      ? ([
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use GENSPARX_* equivalents instead (OPENCLAW_* is still accepted):",
    ...entries.map((key) => {
