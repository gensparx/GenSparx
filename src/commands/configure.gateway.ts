...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "GENSPARX_GATEWAY_TOKEN",
          placeholder: "GENSPARX_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like GENSPARX_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. gensparx will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: gensparx trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.gensparx.com/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
