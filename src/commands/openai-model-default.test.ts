...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: GensparxConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: GensparxConfig) => { changed: boolean; next: GensparxConfig };
  defaultModel: string;
  overrideConfig: GensparxConfig;
  alreadyDefaultConfig: GensparxConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: GensparxConfig) => config,
      applyDefaultConfig: (config: GensparxConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: GensparxConfig = { agents: { defaults: {} } };
