...
type PromptDefaultModelResult = { model?: string; config?: GensparxConfig };
...
function resolveConfiguredModelRaw(cfg: GensparxConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: GensparxConfig): string[] {
...
export function applyPrimaryModel(cfg: GensparxConfig, model: string): GensparxConfig {
...
export function applyModelAllowlist(cfg: GensparxConfig, models: string[]): GensparxConfig {
  const defaults = cfg.agents?.defaults;
