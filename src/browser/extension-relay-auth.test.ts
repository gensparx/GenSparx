...
import {
  probeAuthenticatedGensparxRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedGensparxRelay({
    baseUrl,
    relayAuthHeader: "x-gensparx-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated gensparx relay probe responses", async () => {
