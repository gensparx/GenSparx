...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".gensparx-profile-decorated");
}
 ...
 */
export function decorateGensparxProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_GENSPARX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_GENSPARX_BROWSER_COLOR).toUpperCase();
