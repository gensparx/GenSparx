...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredGensparxTmpDir: vi.fn(() => "/tmp/gensparx"),
}));
vi.mock("../infra/tmp-gensparx-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredGensparxTmpDir.mockReturnValue("/tmp/gensparx");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".gensparx-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredGensparxTmpDir.mockReturnValue("/tmp/gensparx-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "gensparx-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "gensparx-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredGensparxTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "gensparx-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "gensparx-preferred", "downloads")}${path.sep}`),
    );
  });
