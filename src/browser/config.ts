import type { BrowserConfig, BrowserProfileConfig, GensparxConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_GENSPARX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_GENSPARX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "gensparx" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_GENSPARX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_GENSPARX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an gensparx browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for gensparx/gensparx CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: GensparxConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_GENSPARX_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_GENSPARX_BROWSER_PROFILE_NAME]
        ? DEFAULT_GENSPARX_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "gensparx";
