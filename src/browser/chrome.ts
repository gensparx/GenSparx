...
export function resolveGensparxUserDataDir(profileName = DEFAULT_GENSPARX_BROWSER_PROFILE_NAME) {
...
export async function launchGensparxChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_GENSPARX_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateGensparxProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
...
export async function stopGensparxChrome(
