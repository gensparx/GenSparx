...
vi.mock("./chrome.js", () => ({
  resolveGenSparxUserDataDir: vi.fn(() => "/tmp/gensparx-test/gensparx/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveGenSparxUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "gensparx-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveGenSparxUserDataDir).mockReturnValue(userDataDir);
