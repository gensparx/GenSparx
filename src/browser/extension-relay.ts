...
const RELAY_AUTH_HEADER = "x-gensparx-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Gensparx-Extension-Relay",
            revision: "0",
            userAgent: "Gensparx-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Gensparx/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedGensparxRelay({
