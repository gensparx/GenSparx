...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/gensparx" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          gensparx: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.GENSPARX_GATEWAY_PORT;
  process.env.GENSPARX_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.GENSPARX_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.GENSPARX_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.GENSPARX_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.GENSPARX_GATEWAY_PASSWORD;
  } else {
    process.env.GENSPARX_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
