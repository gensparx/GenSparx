...
function localGensparxProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "gensparx",
...
function createLocalGensparxResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localGensparxProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveGensparxUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-reset-"));
    const profileDir = path.join(tempRoot, "gensparx");
    ...
    const ops = createLocalGensparxResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveGensparxUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "gensparx-reset-no-own-"));
    const profileDir = path.join(tempRoot, "gensparx");
    fs.mkdirSync(profileDir, { recursive: true });
