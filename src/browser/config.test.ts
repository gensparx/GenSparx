...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("gensparx");
    expect(profile?.driver).toBe("gensparx");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const gensparx = resolveProfile(resolved, "gensparx");
    expect(gensparx?.driver).toBe("gensparx");
    expect(gensparx?.cdpPort).toBe(18800);
    expect(gensparx?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from GENSPARX_GATEWAY_PORT when unset", () => {
    withEnv({ GENSPARX_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const gensparx = resolveProfile(resolved, "gensparx");
      expect(gensparx?.cdpPort).toBe(19012);
      expect(gensparx?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ GENSPARX_GATEWAY_PORT: undefined }, () => {
      ...
      const gensparx = resolveProfile(resolved, "gensparx");
      expect(gensparx?.cdpPort).toBe(19022);
      expect(gensparx?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(gensparx?.cdpPort).toBe(19000);
    expect(gensparx?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "gensparx");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        gensparx: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("gensparx");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to gensparx profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps gensparx default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("gensparx");
    ...
    it("keeps gensparx default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("gensparx");
    ...
    it("keeps gensparx default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
