...
var GensparxA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--gensparx-a2ui-inset-top, 0px)
        var(--gensparx-a2ui-inset-right, 0px)
        var(--gensparx-a2ui-inset-bottom, 0px)
        var(--gensparx-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--gensparx-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--gensparx-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--gensparx-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--gensparx-a2ui-empty-top, var(--gensparx-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		globalThis.gensparxA2UI = api;
		this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
		this.#statusListener = (evt) => this.#handleActionStatus(evt);
		for (const eventName of ["gensparx:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["gensparx:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__gensparxLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.gensparxCanvasA2UIAction ?? globalThis.gensparxCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.gensparxCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("gensparx-a2ui-host")) customElements.define("gensparx-a2ui-host", GensparxA2UIHost);
