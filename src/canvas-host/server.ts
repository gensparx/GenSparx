...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.gensparxCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.gensparxCanvasA2UIAction &&
        typeof window.gensparxCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.gensparxSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>GenSparx Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
