...
let originalGensparxLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.GENSPARX_TEST_FILE_LOG;
  originalGensparxLogLevel = process.env.GENSPARX_LOG_LEVEL;
  delete process.env.GENSPARX_TEST_FILE_LOG;
  delete process.env.GENSPARX_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.GENSPARX_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalGensparxLogLevel === undefined) {
    delete process.env.GENSPARX_LOG_LEVEL;
  } else {
    process.env.GENSPARX_LOG_LEVEL = originalGensparxLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
