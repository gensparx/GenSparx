...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "GENSPARX_SKIP_BROWSER_CONTROL_SERVER",
  "GENSPARX_SKIP_GMAIL_WATCHER",
  "GENSPARX_SKIP_CANVAS_HOST",
  "GENSPARX_BUNDLED_PLUGINS_DIR",
  "GENSPARX_SKIP_CHANNELS",
  "GENSPARX_SKIP_PROVIDERS",
  "GENSPARX_SKIP_CRON",
  "GENSPARX_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.GENSPARX_STATE_DIR = path.join(tempHome, ".gensparx");
...
function applyGatewaySkipEnv() {
  ...
  process.env.GENSPARX_SKIP_PROVIDERS = "1";
  process.env.GENSPARX_SKIP_CRON = "1";
  process.env.GENSPARX_TEST_MINIMAL_GATEWAY = "1";
  process.env.GENSPARX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "gensparx-test-no-bundled-extensions")
    : "gensparx-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".gensparx-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".gensparx-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__gensparxTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__gensparxTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["GENSPARX_GATEWAY_TOKEN"]);
  const prev = process.env.GENSPARX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.GENSPARX_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.GENSPARX_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
