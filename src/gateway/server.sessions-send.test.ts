...
const { createGensparxTools } = await import("../agents/gensparx-tools.js");
...
type SessionSendTool = ReturnType<typeof createGensparxTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createGensparxTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["GENSPARX_GATEWAY_PORT", "GENSPARX_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.GENSPARX_GATEWAY_PORT = String(gatewayPort);
  process.env.GENSPARX_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("GENSPARX_CONFIG_PATH missing in gateway test environment");
      }
