...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("gensparx doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with GENSPARX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with GENSPARX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.GENSPARX_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://gensparx-gateway.ai:18789",
      onConnectError,
    });
