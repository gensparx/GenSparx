...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-gensparx-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-gensparx-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-gensparx-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-gensparx-thread-id")?.trim() || undefined;
  ...
  const allTools = createGensparxTools({
