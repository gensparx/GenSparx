...
export function resolveHooksConfig(cfg: GensparxConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: GensparxConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-gensparx-token"] === "string"
      ? req.headers["x-gensparx-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
