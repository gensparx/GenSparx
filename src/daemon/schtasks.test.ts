...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when GENSPARX_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".gensparx", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when GENSPARX_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", GENSPARX_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".gensparx-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers GENSPARX_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        GENSPARX_PROFILE: "rescue",
        GENSPARX_STATE_DIR: "C:\\State\\gensparx",
      },
      expected: path.join("C:\\State\\gensparx", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", GENSPARX_PROFILE: "default" },
      expected: path.join("/home/test", ".gensparx", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem gensparx Gateway",
          "cd /d C:\\Projects\\gensparx",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\gensparx",
          environment: {
            NODE_ENV: "production",
            GENSPARX_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\gensparx\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\gensparx\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\gensparx Share\\node.exe" "\\\\fileserver\\gensparx Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\gensparx Share\\node.exe",
            "\\\\fileserver\\gensparx Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from GENSPARX_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ GENSPARX_STATE_DIR: path.join(tmpDir, "custom-state") }),
