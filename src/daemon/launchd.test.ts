...
async function withLaunchctlStub(
  ...
  const originalListOutput = process.env.GENSPARX_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.GENSPARX_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.GENSPARX_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.GENSPARX_TEST_LAUNCHCTL_LOG = logPath;
    process.env.GENSPARX_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        GENSPARX_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.GENSPARX_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.GENSPARX_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.GENSPARX_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.GENSPARX_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 ai.gensparx.gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when GENSPARX_PROFILE is default", () => {
    const env = { HOME: "/Users/test", GENSPARX_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.gensparx.gateway.plist",
    );
  });

  it("uses default label when GENSPARX_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when GENSPARX_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", GENSPARX_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.gensparx.jbphoenix.plist",
    );
  });

  it("prefers GENSPARX_LAUNCHD_LABEL over GENSPARX_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      GENSPARX_PROFILE: "jbphoenix",
      GENSPARX_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from GENSPARX_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      GENSPARX_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty GENSPARX_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      GENSPARX_PROFILE: "myprofile",
      GENSPARX_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.gensparx.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from GENSPARX_PROFILE", () => {
    const env = { HOME: "/Users/test", GENSPARX_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.gensparx.myprofile.plist",
    );
  });
