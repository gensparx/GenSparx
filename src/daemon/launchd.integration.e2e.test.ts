...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `gensparx-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      GENSPARX_LAUNCHD_LABEL: `ai.gensparx.launchd-int-${testId}`,
      GENSPARX_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
