...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { GENSPARX_PROFILE: "work", GENSPARX_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { GENSPARX_PROFILE: "work", GENSPARX_SERVICE_VERSION: "local" },
        environment: { GENSPARX_SERVICE_VERSION: "remote" },
      }),
    ).toBe("gensparx Gateway (profile: work, vremote)");
  });
});
