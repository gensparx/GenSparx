...
`;
    ...
    expect(parsed.gensparx.emoji).toBe("💾");
    expect(parsed.gensparx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.gensparx.emoji).toBe("📝");
    expect(parsed.gensparx.events).toEqual(["command"]);
    expect(parsed.gensparx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.gensparx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"gensparx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"gensparx": {"events": ["test"]}}');
  });
...
describe("resolveGensparxMetadata", () => {
  it("extracts gensparx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveGensparxMetadata(frontmatter);
  ...
  it("returns undefined when gensparx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        gensparx: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Gensparx" },
            { id: "npm", kind: "npm", package: "@gensparx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@gensparx/hook");
  });
...
`;
    ...
    const gensparx = resolveGensparxMetadata(frontmatter);
    expect(gensparx).toBeDefined();
    expect(gensparx?.emoji).toBe("💾");
    expect(gensparx?.events).toEqual(["command:new", "command:reset"]);
    expect(gensparx?.requires?.config).toEqual(["workspace.dir"]);
    expect(gensparx?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const gensparx = resolveGensparxMetadata(frontmatter);
    expect(gensparx?.emoji).toBe("disk");
    expect(gensparx?.events).toEqual(["command:new"]);
  });
});
