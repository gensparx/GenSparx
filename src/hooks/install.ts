...
async function ensureOpenClawHooks(manifest: HookPackageManifest) {
  const hooks =
    ...
    LEGACY_MANIFEST_KEYS.map((key) => manifest[key]?.hooks).find(Array.isArray);
  if (!Array.isArray(hooks)) {
    throw new Error(`package.json missing ${MANIFEST_KEY}.hooks or ${LEGACY_MANIFEST_KEYS[0]}.hooks`);
  }
