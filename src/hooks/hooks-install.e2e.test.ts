...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.GENSPARX_STATE_DIR;
    prevBundledDir = process.env.GENSPARX_BUNDLED_HOOKS_DIR;
    process.env.GENSPARX_STATE_DIR = path.join(baseDir, "state");
    process.env.GENSPARX_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.GENSPARX_BUNDLED_HOOKS_DIR;
    } else {
      process.env.GENSPARX_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          gensparx: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
