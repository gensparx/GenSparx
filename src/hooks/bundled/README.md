...
This directory contains hooks that ship with GenSparx. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.gensparx.com/hooks/soul-evil
...
Hooks can be configured in `~/.GenSparx/GenSparx.json`:
...
2. Restart gateway: `pkill -9 -f 'GenSparx.*gateway' && pnpm GenSparx gateway`
3. Enable the hook: `GenSparx hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.gensparx.com/hooks
