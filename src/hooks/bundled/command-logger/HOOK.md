...
homepage: https://docs.gensparx.com/automation/hooks#command-logger
metadata:
  {
    "gensparx":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.gensparx/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.gensparx/logs/commands.log ~/.gensparx/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/gensparx`:
   ```
   /home/username/.gensparx/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
