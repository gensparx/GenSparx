...
describe("shared/frontmatter", () => {
  ...
  test("resolveGensparxManifestBlock parses JSON5 metadata and picks gensparx block", () => {
    const frontmatter = {
      metadata: "{ gensparx: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveGensparxManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveGensparxManifestBlock returns undefined for invalid input", () => {
    expect(resolveGensparxManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveGensparxManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveGensparxManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
