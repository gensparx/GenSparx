...
const originalBundledDir = process.env.GENSPARX_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("gensparx-bundled-");
  const workspaceDir = await createTempDir("gensparx-workspace-");
...
afterEach(async () => {
  process.env.GENSPARX_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.GENSPARX_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
