...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; GenSparx handles all routing internally.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running GenSparx process",
  ...
  const displayWorkspaceDir = sanitizeForPromptLiteral(
    params.sandboxInfo?.enabled
      ? params.sandboxInfo.containerWorkspaceDir ?? params.workspaceDir
      : params.workspaceDir,
  );
  const hostWorkspaceDir = sanitizeForPromptLiteral(params.workspaceDir);
  const workspaceGuidance =
    params.sandboxInfo?.enabled && params.sandboxInfo.containerWorkspaceDir
      ? `For read/write/edit/apply_patch, file paths resolve against host workspace: ${hostWorkspaceDir}. For bash/exec commands, use sandbox container paths under ${sanitizeForPromptLiteral(params.sandboxInfo.containerWorkspaceDir)} (or relative paths from that workdir), not host paths.`
      : "Use relative paths from this working directory for file operations.";
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control gensparx's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "If unsure, ask the user to run `gensparx help` (or `gensparx gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    ...
    "These user-editable files are loaded by GenSparx and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'GenSparx treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
