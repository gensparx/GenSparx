...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending Gensparx's
...
function resolveExecConfig(params: { cfg?: GensparxConfig; agentId?: string }) {
...
export function createGensparxCodingTools(options?: {
  agentId?: string;
  ...
  /** Optional snapshot of resolved skill roots allowed for read-only access. */
  ...
  const additionalReadRoots = Array.from(
    new Set(
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createGensparxReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
      return [
