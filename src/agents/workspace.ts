...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.GENSPARX_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".gensparx", `workspace-${profile}`);
  }
  return path.join(home, ".gensparx", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".gensparx";
