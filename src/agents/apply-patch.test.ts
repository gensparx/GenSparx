...
async function createSymlinkOrSkip(target: string, linkPath: string): Promise<boolean> {
  try {
...
*** End Patch`;
  ...
  it("rejects symlink escape attempts by default", async () => {
    await withTempDir(async (dir) => {
      ...
      if (!(await createSymlinkOrSkip(outside, linkPath))) {
...
*** End Patch`;
  ...
  it("allows symlinks that resolve within cwd by default", async () => {
    await withTempDir(async (dir) => {
      ...
      if (!(await createSymlinkOrSkip(target, linkPath))) {
        return;
      }
...
*** End Patch`;
  ...
  it("rejects delete path traversal via symlink directories by default", async () => {
    await withTempDir(async (dir) => {
      ...
      if (!(await createSymlinkOrSkip(outsideDir, linkDir))) {
