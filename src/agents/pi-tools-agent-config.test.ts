...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: GensparxConfig) {
    return createGensparxCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<GensparxConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<GensparxConfig["agents"]>["list"]>[number]["tools"];
  }): GensparxConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createGensparxCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createGensparxCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createGensparxCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createGensparxCodingTools({
      config: cfg,
    ...
    const bobTools = createGensparxCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createGensparxCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createGensparxCodingTools({
      config: cfg,
