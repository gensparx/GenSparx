...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): GensparxConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as GensparxConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): GensparxConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as GensparxConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: GensparxConfig, accountId = "media") {
    await handleTelegramAction(
