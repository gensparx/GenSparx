...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    gensparx: process.env.GENSPARX_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.gensparx === undefined) {
      delete process.env.GENSPARX_GATEWAY_TOKEN;
    } else {
      process.env.GENSPARX_GATEWAY_TOKEN = envSnapshot.gensparx;
    }
  ...
  it("uses GENSPARX_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.GENSPARX_GATEWAY_TOKEN = "local-env-token";
