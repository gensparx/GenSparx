...
function installMockFetch(payload: unknown) {
  ...
  vi.stubGlobal("fetch", withFetchPreconnect(mockFetch as unknown as typeof fetch));
  return mockFetch;
}
...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "gensparx" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "Gensparx", url: "https://gensparx.com/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest gensparx release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://gensparx.com/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://gensparx.com/docs"]);
