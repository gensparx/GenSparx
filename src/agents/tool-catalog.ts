...
type CoreToolDefinition = {
  ...
  includeInGensparxGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const gensparxTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInGensparxGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:gensparx": gensparxTools,
