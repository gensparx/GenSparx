...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/gensparx/gensparx/issues/31298
    previousFastTestEnv = process.env.GENSPARX_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.GENSPARX_TEST_FAST;
      return;
    }
    process.env.GENSPARX_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // GENSPARX_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("gensparx runtime context (internal):");
