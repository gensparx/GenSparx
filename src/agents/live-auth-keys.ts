...
const GOOGLE_LIVE_SINGLE_KEY = "GENSPARX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "GENSPARX_LIVE_ANTHROPIC_KEY",
    listVar: "GENSPARX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "GENSPARX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `GENSPARX_LIVE_${base}_KEY`;
