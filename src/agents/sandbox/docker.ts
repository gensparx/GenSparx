...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "gensparx.sandbox=1");
  args.push("--label", `gensparx.sessionKey=${params.scopeKey}`);
  args.push("--label", `gensparx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `gensparx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "gensparx.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`gensparx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`gensparx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("gensparx sandbox recreate --all");
}
