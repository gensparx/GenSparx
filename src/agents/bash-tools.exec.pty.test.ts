...
test("exec sets GENSPARX_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-gensparx-shell", {
    command: "node -e \"process.stdout.write(process.env.GENSPARX_SHELL || '')\"",
    pty: true,
  });
