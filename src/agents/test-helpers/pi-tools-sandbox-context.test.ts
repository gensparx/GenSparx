...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("gensparx-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("gensparx-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("gensparx-sbx-");
