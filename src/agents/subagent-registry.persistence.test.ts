...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["GENSPARX_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-subagent-"));
  ...
  it("uses isolated temp state when GENSPARX_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "gensparx-test-state"));
  });
});
