...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-embedded-agent-"));
...
const ensureModels = (cfg: GenSparxConfig) => ensureGenSparxModelsJson(cfg, agentDir) as unknown;
