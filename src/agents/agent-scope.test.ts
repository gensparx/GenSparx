...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: GensparxConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/gensparx" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: GensparxConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: GensparxConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: GensparxConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: GensparxConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: GensparxConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: GensparxConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: GensparxConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/gensparx");
  });

  it("uses GENSPARX_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "gensparx-home");
    vi.stubEnv("GENSPARX_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as GensparxConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".gensparx", "workspace"));
  });

  it("uses GENSPARX_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "gensparx-home");
    vi.stubEnv("GENSPARX_HOME", home);
    // Clear state dir so it falls back to GENSPARX_HOME
    vi.stubEnv("GENSPARX_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as GensparxConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".gensparx", "agents", "main", "agent"));
  });
});
