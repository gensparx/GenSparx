...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses GENSPARX_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      GENSPARX_HOME: "/srv/gensparx-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/gensparx-home"), ".gensparx", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".gensparx", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("gensparx-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-workspace-hardlink-"));
    try {
