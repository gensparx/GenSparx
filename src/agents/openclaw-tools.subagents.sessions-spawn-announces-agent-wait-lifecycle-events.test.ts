...
describe("gensparx-tools: subagents", () => {
  beforeEach(() => {
    configOverride = {
      session: {
        mainKey: "main",
  ...
  it("sessions_spawn deletes session when cleanup=delete via agent.wait", async () => {
    ...
    // Current flow guarantees subagent spawn; main agent trigger may be hook/routing dependent.
    ...
    // Optional second call: main agent trigger
    ...
    // Session deletion may be performed by lifecycle hooks depending on runtime wiring.
    if (deletedKey) {
