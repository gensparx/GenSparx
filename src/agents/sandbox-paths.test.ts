...
import { resolvePreferredGensparxTmpDir } from "../infra/tmp-gensparx-dir.js";
...
async function withOutsideHardlinkInGensparxTmp<T>(
  params: {
    gensparxTmpDir: string;
  ...
  const hardlinkPath = path.join(params.gensparxTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.gensparxTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.gensparxTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.gensparxTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const gensparxTmpDir = resolvePreferredGensparxTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred gensparx tmp root",
      media: path.join(gensparxTmpDir, "image.png"),
      expected: path.join(gensparxTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred gensparx tmp root",
      media: pathToFileURL(path.join(gensparxTmpDir, "photo.png")).href,
      expected: path.join(gensparxTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred gensparx tmp root",
      media: path.join(gensparxTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(gensparxTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(gensparxTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside gensparx tmp root",
      media: path.join(os.tmpdir(), "outside-gensparx", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked gensparx tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(gensparxTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(gensparxTmpDir, { recursive: true });
      const symlinkPath = path.join(gensparxTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked gensparx tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGensparxTmp(
      {
        gensparxTmpDir,
  ...
  it("rejects symlinked gensparx tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGensparxTmp(
