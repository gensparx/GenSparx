...
describe("resolveGensparxAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("honors GENSPARX_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          GENSPARX_STATE_DIR: undefined,
          GENSPARX_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveGensparxAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when GENSPARX_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers GENSPARX_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          GENSPARX_STATE_DIR: undefined,
          GENSPARX_AGENT_DIR: primaryOverride,
