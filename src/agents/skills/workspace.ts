...
function resolveSkillsLimits(config?: GensparxConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "gensparx-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "gensparx-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveGensparxMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: GensparxConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`gensparx skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
