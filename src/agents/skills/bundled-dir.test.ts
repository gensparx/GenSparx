...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns GENSPARX_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-bundled-override-"));
    process.env.GENSPARX_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
