---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for gensparx deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, gensparx cron scheduling for periodic checks, or version status checks on a machine running gensparx (laptop, workstation, Pi, VPS).
---

# gensparx Host Hardening

## Overview

Assess and harden the host running Gensparx, then align it to a user-defined risk tolerance without breaking access. Use gensparx security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim gensparx changes the host firewall, SSH, or OS updates; it does not.
...
5. gensparx gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if gensparx can access sensitive data.
...
### 2) Run gensparx security audits (read-only)

As part of the default read-only checks, run `gensparx security audit --deep`. Only offer alternatives if the user requests them:

1. `gensparx security audit` (faster, non-probing)
2. `gensparx security audit --json` (structured output)

Offer to apply gensparx safe defaults (numbered):

1. `gensparx security audit --fix`

Be explicit that `--fix` only tightens gensparx defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check gensparx version/update status (read-only)

As part of the default read-only checks, run `gensparx update status`.
...
- Credential hygiene notes (location of gensparx creds, prefer disk encryption)
...
- gensparx security audit (re-run)
...
After gensparx install or first hardening pass, run at least one baseline audit and version check:

- `gensparx security audit`
- `gensparx security audit --deep`
- `gensparx update status`

Ongoing monitoring is recommended. Use the gensparx cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `gensparx cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `gensparx update status`
...
Before creating, `gensparx cron list` and match on exact `name`. If found, `gensparx cron edit <id> ...`.
If not found, `gensparx cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `gensparx update status` (preferred for source checkouts and channels)
2. `npm view gensparx version` (published npm version)

## gensparx command accuracy

Use only supported commands and flags:

- `gensparx security audit [--deep] [--fix] [--json]`
- `gensparx status` / `gensparx status --deep`
- `gensparx health --json`
- `gensparx update status`
- `gensparx cron add|list|runs|run`

Do not invent CLI flags or imply gensparx enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by gensparx compaction:
