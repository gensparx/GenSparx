---
name: node-connect
description: Diagnose Gensparx node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Gensparx is advertising that route, then fix pairing/auth.
...
- whether `gensparx devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `gensparx qr --json`. It uses the same setup-code payload Android scans.

```bash
gensparx config get gateway.mode
gensparx config get gateway.bind
gensparx config get gateway.tailscale.mode
gensparx config get gateway.remote.url
gensparx config get gateway.auth.mode
gensparx config get gateway.auth.allowTailscale
gensparx config get plugins.entries.device-pair.config.publicUrl
gensparx qr --json
gensparx devices list
gensparx nodes status
```

If this Gensparx instance is pointed at a remote gateway, also run:

```bash
gensparx qr --remote --json
```
...
`gensparx qr --json` success means:
...
If `gensparx qr --json` says `Gateway is only bound to loopback`:
...
- `gensparx devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run gensparx qr again, rescan, then approve the pending device pairing.`
