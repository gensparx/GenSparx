...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `gensparx-e2e-${name}-`));
  const configDir = path.join(homeDir, ".gensparx");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          GENSPARX_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        GENSPARX_GATEWAY_TOKEN: inst.gatewayToken,
        GENSPARX_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          GENSPARX_GATEWAY_PORT: String(gwA.port),
          GENSPARX_GATEWAY_TOKEN: gwA.gatewayToken,
          GENSPARX_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          GENSPARX_GATEWAY_PORT: String(gwB.port),
          GENSPARX_GATEWAY_TOKEN: gwB.gatewayToken,
          GENSPARX_GATEWAY_PASSWORD: "",
        }),
