...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked GENSPARX_HOME.
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "gensparx-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".gensparx", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
