...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("gensparx-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("gensparx-bin-sherpa-");
      const modelDir = await makeTempDir("gensparx-sherpa-model-");
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("gensparx-bin-whispercpp-");
      const modelDir = await makeTempDir("gensparx-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("gensparx-bin-gemini-");
      ...
      const cfg: GenSparxConfig = { tools: { media: { image: {} } } };
