...
export async function handleWhatsAppWait(host: GenSparxApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: GenSparxApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: GenSparxApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: GenSparxApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: GenSparxApp): string {
...
function resolveGatewayHttpAuthHeader(host: GenSparxApp): string | null {
...
function buildGatewayHttpHeaders(host: GenSparxApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: GenSparxApp) {
...
export function handleNostrProfileToggleAdvanced(host: GenSparxApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: GenSparxApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: GenSparxApp) {
  const state = host.nostrProfileFormState;
