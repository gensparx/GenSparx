...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/gensparx/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/gensparx");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/gensparx/cron");
  });
  ...
  it("honors explicit base path overrides", async () => {
    ...
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/gensparx/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("gensparx.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "gensparx.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/gensparx" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("gensparx.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/gensparx",
    });
    expect(JSON.parse(localStorage.getItem("gensparx.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/gensparx";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/gensparx");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/gensparx#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/gensparx");
    expect(app.settings.token).toBe("");
