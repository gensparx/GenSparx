...
show_usage() {
    cat << EOF
clawlog - gensparx Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View gensparx logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    gensparx logs flow through the macOS unified log (subsystem: ai.gensparx).
