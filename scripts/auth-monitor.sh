...
#
# Suggested cron: */30 * * * * /home/admin/gensparx/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send GenSparx notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., gensparx-alerts)
...
STATE_FILE="$HOME/.gensparx/auth-monitor-state"
...
send_notification() {
    ...
    # Send via GenSparx if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use gensparx
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via GenSparx to $NOTIFY_PHONE..."
            gensparx send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! GenSparx is down. Run: ssh l36 '~/gensparx/scripts/mobile-reauth.sh'" "urgent"
    exit 1
