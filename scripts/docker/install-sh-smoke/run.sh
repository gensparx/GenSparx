...
INSTALL_URL="${GENSPARX_INSTALL_URL:-https://gensparx.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${GENSPARX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${GENSPARX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="gensparx"
PACKAGE_NAME="${GENSPARX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (GENSPARX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${GENSPARX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${GENSPARX_INSTALL_LATEST_OUT:-}"
fi
