...
# Build and bundle gensparx into a minimal .app we can open.
# Outputs to dist/Gensparx.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Gensparx.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/gensparx/gensparx/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Gensparx/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :GensparxBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :GensparxGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Gensparx"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Gensparx"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Gensparx"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Gensparx" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Gensparx/Resources/Gensparx.icns" "$APP_ROOT/Contents/Resources/Gensparx.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Gensparx/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying GensparxKit resources"
GENSPARXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/GensparxKit_GensparxKit.bundle"
if [ -d "$GENSPARXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/GensparxKit_GensparxKit.bundle"
  cp -R "$GENSPARXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/GensparxKit_GensparxKit.bundle"
else
  echo "WARN: GensparxKit resource bundle not found at $GENSPARXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Gensparx"
killall -q gensparx 2>/dev/null || true
