...
const shouldBuild = (deps) => {
  if (deps.env.GENSPARX_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.GENSPARX_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[gensparx] ${message}\n`);
};

const runGensparx = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["gensparx.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
