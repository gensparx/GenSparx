...
type DriverMode = "token" | "webhook" | "gensparx";
...
function resolveStateDir(): string {
  const override = process.env.GENSPARX_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.GENSPARX_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".gensparx");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver gensparx] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) gensparx spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|gensparx> Driver transport mode (default: token)\n" +
    ...
    "  --gensparx-bin <path>        gensparx CLI binary for driver=gensparx (default: gensparx)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  GENSPARX_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  GENSPARX_DISCORD_SMOKE_DRIVER\n" +
    "  GENSPARX_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  GENSPARX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  GENSPARX_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  GENSPARX_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  GENSPARX_DISCORD_SMOKE_AGENT\n" +
    "  GENSPARX_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  GENSPARX_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  GENSPARX_DISCORD_SMOKE_POLL_MS\n" +
    "  GENSPARX_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  GENSPARX_DISCORD_SMOKE_GENSPARX_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.GENSPARX_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.GENSPARX_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.GENSPARX_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.GENSPARX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.GENSPARX_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.GENSPARX_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.GENSPARX_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.GENSPARX_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.GENSPARX_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.GENSPARX_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.GENSPARX_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.GENSPARX_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const gensparxBin =
    resolveArg("--gensparx-bin") || process.env.GENSPARX_DISCORD_SMOKE_GENSPARX_BIN || "gensparx";
...
async function gensparxCliJson<T>(params: { gensparxBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.gensparxBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`gensparx ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithGensparx(params: {
  ...
  const response = await gensparxCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    gensparxBin: params.gensparxBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `gensparx-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await gensparxCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("gensparx message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Gensparx.`,
        diagnostics: {
          bindingCandidates: [
            {
