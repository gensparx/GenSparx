#!/bin/bash
# Setup gensparx Auth Management System
...
echo "=== gensparx Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'gensparx-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  gensparx message: Send warning via gensparx itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/gensparx-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/gensparx-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/gensparx-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now gensparx-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status gensparx-auth-monitor.timer"
echo ""
echo "Quick commands:"
