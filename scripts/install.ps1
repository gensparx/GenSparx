...
param(
    ...
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\gensparx",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  ⚡ gensparx Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Gensparx.$NC" -Level info
    Write-Host ""
}
...
function Install-GensparxNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing gensparx (gensparx@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g gensparx@$Version --no-fund --no-audit 2>&1
        Write-Host "gensparx installed" -Level success
        return $true
    } catch {
...
function Install-GensparxGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing gensparx from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/gensparx/gensparx.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\gensparx\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\gensparx.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install gensparx from git to $GitDir" -Level info
        } else {
            Install-GensparxGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install gensparx via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-GensparxNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'gensparx onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "⚡ gensparx installed successfully!" -Level success
}
