 ...
 * `gensparx update`. Use it to verify the completion installation flow
...
\`gensparx update\`, \`gensparx doctor\`, and \`gensparx onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# gensparx Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`gensparx completion --install\` later to enable.`));
    return;
  }
