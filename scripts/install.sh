...
DEFAULT_TAGLINE="All your chats, one Gensparx."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${GENSPARX_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "⚡ gensparx Installer")"
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$GENSPARX_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
cleanup_npm_gensparx_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.gensparx-* "$npm_root"/gensparx 2>/dev/null || true
}

extract_gensparx_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_gensparx_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/gensparx" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/gensparx"|"/usr/local/bin/gensparx")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/gensparx/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale gensparx symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing gensparx binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing gensparx package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_gensparx_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "gensparx npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*gensparx" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_gensparx_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_gensparx_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "gensparx npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an gensparx binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "GENSPARX_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "GENSPARX_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "GENSPARX_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "GENSPARX_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "GENSPARX_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "GENSPARX_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "GENSPARX_BETA" "CLAWDBOT_BETA"
map_legacy_env "GENSPARX_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "GENSPARX_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "GENSPARX_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "GENSPARX_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "GENSPARX_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "GENSPARX_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${GENSPARX_TAGLINE_INDEX:-}" ]]; then
        if [[ "${GENSPARX_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((GENSPARX_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${GENSPARX_NO_ONBOARD:-0}
NO_PROMPT=${GENSPARX_NO_PROMPT:-0}
DRY_RUN=${GENSPARX_DRY_RUN:-0}
INSTALL_METHOD=${GENSPARX_INSTALL_METHOD:-}
GENSPARX_VERSION=${GENSPARX_VERSION:-latest}
USE_BETA=${GENSPARX_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/gensparx"
GIT_DIR=${GENSPARX_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${GENSPARX_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${GENSPARX_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${GENSPARX_VERBOSE:-0}"
GENSPARX_BIN=""
PNPM_CMD=()
HELP=0
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/gensparx)
...
Environment variables:
  GENSPARX_INSTALL_METHOD=git|npm
  GENSPARX_VERSION=latest|next|<semver>
  GENSPARX_BETA=0|1
  GENSPARX_GIT_DIR=...
  GENSPARX_GIT_UPDATE=0|1
  GENSPARX_NO_PROMPT=1
  GENSPARX_DRY_RUN=1
  GENSPARX_NO_ONBOARD=1
  GENSPARX_VERBOSE=1
  GENSPARX_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected gensparx checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a gensparx source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_gensparx_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"gensparx"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
ensure_gensparx_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/gensparx" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/gensparx" ]]; then
        ln -sf "$npm_root/gensparx/dist/entry.js" "${npm_bin}/gensparx"
        ui_info "Created gensparx bin link at ${npm_bin}/gensparx"
    fi
    return 0
}

# Check for existing gensparx installation
check_existing_gensparx() {
    if [[ -n "$(type -P gensparx 2>/dev/null || true)" ]]; then
        ui_info "Existing gensparx installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make gensparx show as \"command not found\" in new terminals."
...
warn_gensparx_not_found() {
    ui_warn "Installed, but gensparx is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t gensparx 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named gensparx; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_gensparx_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P gensparx 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/gensparx" ]]; then
        echo "${npm_bin}/gensparx"
        return 0
    fi
...
install_gensparx_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/gensparx/gensparx.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing gensparx from git checkout: ${repo_dir}"
    else
        ui_info "Installing gensparx from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Gensparx" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Gensparx" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/gensparx" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/gensparx"
    ui_success "gensparx wrapper installed to \$HOME/.local/bin/gensparx"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view gensparx dist-tags.beta 2>/dev/null || true)"
...
install_gensparx() {
    local package_name="gensparx"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            GENSPARX_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="gensparx"
        else
            GENSPARX_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${GENSPARX_VERSION}" ]]; then
        GENSPARX_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${GENSPARX_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing gensparx v${resolved_version}"
    else
        ui_info "Installing gensparx (${GENSPARX_VERSION})"
    fi
    local install_spec=""
    if [[ "${GENSPARX_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${GENSPARX_VERSION}"
    fi

    if ! install_gensparx_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_gensparx_paths
        install_gensparx_npm "${install_spec}"
    fi

    if [[ "${GENSPARX_VERSION}" == "latest" && "${package_name}" == "gensparx" ]]; then
        if ! resolve_gensparx_bin &> /dev/null; then
            ui_warn "npm install gensparx@latest failed; retrying gensparx@next"
            cleanup_npm_gensparx_paths
            install_gensparx_npm "gensparx@next"
        fi
    fi

    ensure_gensparx_bin_link || true

    ui_success "gensparx installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${GENSPARX_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_gensparx_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (gensparx not on PATH yet)"
        warn_gensparx_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${GENSPARX_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.gensparx/workspace-${profile}"
    else
        echo "${HOME}/.gensparx/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${GENSPARX_CONFIG_PATH:-$HOME/.gensparx/gensparx.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run gensparx onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but gensparx not on PATH; skipping onboarding"
        warn_gensparx_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run gensparx onboard to retry"
        return
    }
}

resolve_gensparx_version() {
    local version=""
    local claw="${GENSPARX_BIN:-}"
    if [[ -z "$claw" ]] && command -v gensparx &> /dev/null; then
        claw="$(command -v gensparx)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/gensparx/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/gensparx/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_gensparx_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found gensparx checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set GENSPARX_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_gensparx; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Gensparx"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g gensparx &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g gensparx 2>/dev/null || true
        ...
        install_gensparx_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/gensparx" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/gensparx"
    ...
    GENSPARX_BIN="$(resolve_gensparx_bin || true)"
    ...
    installed_version=$(resolve_gensparx_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "⚡ gensparx installed successfully (${installed_version})!"
    else
        ui_celebrate "⚡ gensparx installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running gensparx doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                GENSPARX_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                GENSPARX_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                GENSPARX_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run gensparx doctor and gensparx plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run gensparx onboard later"
        else
            local config_path="${GENSPARX_CONFIG_PATH:-$HOME/.gensparx/gensparx.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (gensparx not on PATH yet)"
                    warn_gensparx_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run gensparx onboard to finish setup"
            return 0
        fi
    fi

    if command -v gensparx &> /dev/null; then
        local claw="${GENSPARX_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_gensparx_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (gensparx daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if GENSPARX_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: gensparx daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${GENSPARX_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
