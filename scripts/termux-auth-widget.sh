...
# GenSparx Auth Widget for Termux
...
# Check auth status
termux-toast "Checking GenSparx auth..."

STATUS=$(ssh "$SERVER" 'if [ -x "$HOME/gensparx/scripts/claude-auth-status.sh" ]; then "$HOME/gensparx/scripts/claude-auth-status.sh" simple; else "$HOME/openclaw/scripts/claude-auth-status.sh" simple; fi' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" 'if [ -x "$HOME/gensparx/scripts/claude-auth-status.sh" ]; then "$HOME/gensparx/scripts/claude-auth-status.sh" json; else "$HOME/openclaw/scripts/claude-auth-status.sh" json; fi' 2>&1)
    ...
    CLAUDE_EXPIRING|GENSPARX_EXPIRING|OPENCLAW_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/gensparx/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|GENSPARX_EXPIRED|OPENCLAW_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: GenSparx won't work until re-auth"
                ;;
        esac
        ;;
