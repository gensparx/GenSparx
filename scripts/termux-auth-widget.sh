...
SERVER="${GENSPARX_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking gensparx auth..."

STATUS=$(ssh "$SERVER" '$HOME/gensparx/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/gensparx/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|GENSPARX_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
    ...
    CLAUDE_EXPIRED|GENSPARX_EXPIRED|CLAWDBOT_EXPIRED)
