...
Inspired by Simon Willison's [Running gensparx in Docker](https://til.simonwillison.net/llms/gensparx-docker).
...
| `clawdock-cli <command>`  | Run gensparx CLI commands                      |
...
| `clawdock-cd`        | Jump to the gensparx project directory    |
| `clawdock-config`    | Open the gensparx config directory        |
...
```bash
gensparx channels login --channel whatsapp --verbose
```
...
2. Configure it in the gensparx config
...
- Bash or Zsh shell
- gensparx project (from `docker-setup.sh`)
