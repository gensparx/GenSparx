#!/usr/bin/env bash
# ClawDock - Docker helpers for Gensparx
# Inspired by Simon Willison's "Running gensparx in Docker"
# https://til.simonwillison.net/llms/gensparx-docker
#
# Installation:
...
# Common paths to check for Gensparx
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/gensparx"
  "${HOME}/workspace/gensparx"
  "${HOME}/projects/gensparx"
  "${HOME}/dev/gensparx"
  "${HOME}/code/gensparx"
  "${HOME}/src/gensparx"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "⚡ Found gensparx at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/gensparx"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ gensparx not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/gensparx/gensparx.git ~/gensparx"
    echo "  cd ~/gensparx && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^GENSPARX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d gensparx-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart gensparx-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f gensparx-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec gensparx-gateway \
    bash -c 'echo "alias gensparx=\"./gensparx.mjs\"" > /tmp/.bashrc_gensparx && bash --rcfile /tmp/.bashrc_gensparx'
}
...
clawdock-cli() {
  _clawdock_compose run --rm gensparx-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build gensparx-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "GENSPARX_GATEWAY_TOKEN=$token" gensparx-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" gensparx-gateway \
    bash -c './gensparx.mjs config set gateway.remote.token "$TOKEN" && ./gensparx.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec gensparx-gateway \
    bash -c "./gensparx.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart gensparx-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm gensparx-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec gensparx-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'gensparx config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}⚡ ClawDock - Docker Helpers for Gensparx${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (gensparx alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to gensparx project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.gensparx)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'gensparx channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'gensparx status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.gensparx.com${_CLR_RESET}"
  echo ""
}
