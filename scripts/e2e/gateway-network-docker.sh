...
IMAGE_NAME="gensparx-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="gensparx-net-e2e-$$"
GW_NAME="gensparx-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "GENSPARX_GATEWAY_TOKEN=$TOKEN" \
  -e "GENSPARX_SKIP_CHANNELS=1" \
  -e "GENSPARX_SKIP_GMAIL_WATCHER=1" \
  -e "GENSPARX_SKIP_CRON=1" \
  -e "GENSPARX_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
