...
import type { GensparxPluginApi } from "gensparx/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-diffs-tool-"));
...
function createApi(): GensparxPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as GensparxPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},
