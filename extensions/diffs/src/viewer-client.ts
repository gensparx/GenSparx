...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-gensparx-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-gensparx-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.gensparxDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.gensparxDiffsError = "true";
