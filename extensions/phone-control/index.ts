...
import type { GensparxPluginApi, GensparxPluginService } from "gensparx/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: GensparxPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: GensparxPluginApi["config"]): string[] {
...
export default function register(api: GensparxPluginApi) {
  ...
  const timerService: GensparxPluginService = {
