...
vi.mock("gensparx/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("gensparx/plugin-sdk/diagnostics-otel")>(
    "gensparx/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { GensparxPluginServiceContext } from "gensparx/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "gensparx/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/gensparx-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): GensparxPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("gensparx.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("gensparx.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("gensparx.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("gensparx.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("gensparx.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("gensparx.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("gensparx.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("gensparx.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("gensparx.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("gensparx.webhook.processed");
    expect(spanNames).toContain("gensparx.message.processed");
    expect(spanNames).toContain("gensparx.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["gensparx.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("gensparx.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "gensparx.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["gensparx.reason"]).toBe("string");
    expect(String(attrs?.["gensparx.reason"])).not.toContain(
