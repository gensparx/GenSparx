...
GenSparx extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
gensparx message send --channel zalouser --target <threadId> --message "Hello from GenSparx!"
```
...
```bash
gensparx channels login --channel zalouser              # Login via QR
gensparx channels login --channel zalouser --account work
gensparx channels status --probe
gensparx channels logout --channel zalouser
```
...
```bash
# Text
gensparx message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
gensparx message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `gensparx channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
gensparx channels login --channel zalouser --account work
gensparx message send --channel zalouser --account work --target <id> --message "Hello"
