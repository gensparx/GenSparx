...
`gensparx.invoke --each`) without adding new gensparx code per workflow.
...
This extension depends on gensparx internal modules (the embedded agent runner).
It is intended to ship as a **bundled** gensparx extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.gensparx/extensions` as a standalone plugin directory.
