...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "gensparx/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/gensparx-test-session.json"),
