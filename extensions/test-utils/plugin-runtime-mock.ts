import type { PluginRuntime } from "gensparx/plugin-sdk/test-utils";
import { removeAckReactionAfterReply, shouldAckReaction } from "gensparx/plugin-sdk/test-utils";
...
export function createPluginRuntimeMock(overrides: DeepPartial<PluginRuntime> = {}): PluginRuntime {
  const base: PluginRuntime = {
    ...
    state: {
      resolveStateDir: vi.fn(() => "/tmp/gensparx"),
    },
    subagent: {
      run: vi.fn(),
      waitForRun: vi.fn(),
