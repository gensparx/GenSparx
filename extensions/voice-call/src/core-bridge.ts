...
function resolveGensparxRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.GENSPARX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set GENSPARX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveGensparxRoot(), "dist", "extensionAPI.js");
