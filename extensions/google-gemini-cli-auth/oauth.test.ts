...
vi.mock("gensparx/plugin-sdk/google-gemini-cli-auth", () => ({
  isWSL2Sync: () => false,
...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "GENSPARX_GEMINI_OAUTH_CLIENT_ID",
    "GENSPARX_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.GENSPARX_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.GENSPARX_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@gensparx.com" });
      }
      if (url === LOAD_PROD) {
