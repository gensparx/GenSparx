...
import {
  ...
  type WizardPrompter,
} from "gensparx/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: GENSPARX_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: GensparxConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var GENSPARX_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.GENSPARX_TWITCH_ACCESS_TOKEN?.trim();
