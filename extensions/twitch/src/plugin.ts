/**
 * Twitch channel plugin for Gensparx.
 *
...
import { buildChannelConfigSchema } from "gensparx/plugin-sdk/twitch";
 ...
 * for Gensparx. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: GensparxConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: GensparxConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: GensparxConfig): boolean => {
