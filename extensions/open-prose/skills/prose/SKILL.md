...
metadata: { "gensparx": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == gensparx `sessions_spawn`
- **File I/O** == gensparx `read`/`write`
- **Remote fetch** == gensparx `web_fetch` (or `exec` with curl when POST is required)
