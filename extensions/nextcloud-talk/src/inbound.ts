import {
  ...
  type GensparxConfig,
  type RuntimeEnv,
} from "gensparx/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as GensparxConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as GensparxConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as GensparxConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as GensparxConfig);
