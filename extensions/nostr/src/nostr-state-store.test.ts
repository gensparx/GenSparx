...
import type { PluginRuntime } from "gensparx/plugin-sdk/nostr";
  ...
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        const override = stateEnv.GENSPARX_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".gensparx");
      },
    },
  } as PluginRuntime);
  try {
