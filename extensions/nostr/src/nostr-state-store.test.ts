import type { PluginRuntime } from "GenSparx/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.GENSPARX_STATE_DIR ?? process.env.gensparx_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "GenSparx-nostr-"));
  process.env.GENSPARX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.GENSPARX_STATE_DIR?.trim() || env.gensparx_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".gensparx");
      },
    },
  } as PluginRuntime);
  try {
