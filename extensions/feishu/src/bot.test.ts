import type { GensparxConfig, PluginRuntime, RuntimeEnv } from "gensparx/plugin-sdk/feishu";
...
async function dispatchMessage(params: { cfg: GensparxConfig; event: FeishuMessageEvent }) {
...
describe("resolveBroadcastAgents", () => {
  it("returns agent list when broadcast config has the peerId", () => {
    const cfg = { broadcast: { oc_group123: ["susan", "main"] } } as unknown as GensparxConfig;
  ...
  it("returns null when peerId not in broadcast", () => {
    const cfg = { broadcast: { oc_other: ["susan"] } } as unknown as GensparxConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });

  it("returns null when agent list is empty", () => {
    const cfg = { broadcast: { oc_group123: [] } } as unknown as GensparxConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });
});
