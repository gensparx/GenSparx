import type { AnyAgentTool, GensparxPluginApi } from "gensparx/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: GensparxPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<GensparxPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
