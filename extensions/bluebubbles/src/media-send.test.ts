...
import type { GensparxConfig, PluginRuntime } from "gensparx/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): GensparxConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gensparx-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
