...
- `/usage cost` in chats: local cost summary aggregated from gensparx session logs.
- CLI: `gensparx status --usage` prints a full per-provider breakdown.
- CLI: `gensparx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
