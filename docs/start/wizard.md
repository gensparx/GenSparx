...
2. **Workspace** — Location for agent files (default `~/.gensparx/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `gensparx doctor` first.
</Note>
...
- Default workspaces follow `~/.gensparx/workspace-<agentId>`.
...
- CLI command reference: [`gensparx onboard`](/cli/onboard)
