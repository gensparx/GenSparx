...
read_when:
  - You need detailed behavior for gensparx onboard
...
This page is the full reference for `gensparx onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.gensparx/gensparx.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `gensparx doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.gensparx/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `gensparx pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `gensparx health`.
    - `gensparx status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.gensparx/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.gensparx/agents/<agentId>/agent/auth-profiles.json`
...
`~/.gensparx/credentials/oauth.json` (or `$GENSPARX_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>

## Outputs and internals

Typical fields in `~/.gensparx/gensparx.json`:
...
WhatsApp credentials go under `~/.gensparx/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.gensparx/agents/<agentId>/sessions/`.
...
- Stores it under `~/.gensparx/tools/signal-cli/<version>/`
...
- Command reference: [`gensparx onboard`](/cli/onboard)
