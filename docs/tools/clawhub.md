...
ClawHub is the **public skill registry for Gensparx**. It is a free service: all skills are public, open, and visible to everyone for sharing and reuse. A skill is just a folder with a `SKILL.md` file (plus supporting text files). You can browse skills in the web app or use the CLI to search, install, update, and publish skills.

Site: [clawhub.ai](https://clawhub.ai)

## What ClawHub is

- A public registry for gensparx skills.
...
4. Users browse, download, and install skills in Gensparx.
...
If you want to add new capabilities to your gensparx agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new gensparx session so it picks up the new skill.
...
## How it fits into Gensparx

By default, the CLI installs skills into `./skills` under your current working directory. If a gensparx workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). gensparx loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.gensparx/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches gensparx how to perform a
...
Interested in becoming a moderator? Ask in the gensparx Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to gensparx workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/gensparx/skills` and `~/.gensparx/skills`). This is designed to find older skill installs without extra flags.
