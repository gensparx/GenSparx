---
summary: "Agent tool surface for gensparx (browser, canvas, nodes, message, cron) replacing legacy `gensparx-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `gensparx-*` skills
title: "Tools"
---

# Tools (Gensparx)

gensparx exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `gensparx-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `gensparx.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, gensparx logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`gensparx node run`).
...
gensparx tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `gensparx configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Gensparx-managed browser.
...
- Quick smoke: `gensparx nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `gensparx gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, gensparx uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.gensparx/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, gensparx clamps `tools.sessions.visibility` to `tree`.
