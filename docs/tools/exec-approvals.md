...
- **gateway host** → `gensparx` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.gensparx/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.gensparx/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `gensparx approvals allowlist ...`).
- `gensparx security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `gensparx doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.gensparx/exec-approvals.json` directly.

CLI: `gensparx approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
