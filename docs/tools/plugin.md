...
4. Bundled extensions (shipped with GenSparx, **disabled by default**)

- `<gensparx>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `gensparx plugins enable <id>`. Installed plugins are enabled by default,
...
gensparx plugins install <path>                 # copy a local file/dir into ~/.openclaw/extensions/<id>
...
gensparx plugins install @openclaw/voice-call # install from npm
...
- `gensparx plugins install <npm-spec>` uses `npm pack`, extracts into `~/.openclaw/extensions/<id>/`, and enables it in config.
