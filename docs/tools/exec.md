...
- `gateway`/`node` approvals are controlled by `~/.gensparx/exec-approvals.json`.
...
- gensparx sets `GENSPARX_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  gensparx prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`gensparx security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `gensparx doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
