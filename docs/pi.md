...
This document describes how gensparx integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

gensparx uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, gensparx directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Gensparx's local TUI mode)                                             |
...
├── pi-tools.ts                    # createGensparxCodingTools()
...
├── gensparx-tools.ts              # Gensparx-specific tools
...
2. **Custom Replacements**: gensparx replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **gensparx Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Gensparx's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, gensparx CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
gensparx wraps this with `guardSessionManager()` for tool result safety.
...
gensparx maintains an auth profile store with multiple API keys per provider:
...
gensparx loads custom pi extensions for specialized behavior:
...
gensparx also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | gensparx Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom gensparx tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.gensparx/agents/<agentId>/sessions/` (or `$GENSPARX_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `GENSPARX_LIVE_TEST=1`)
