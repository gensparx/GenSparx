...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `gensparx.browserConfigEpoch`) and recommends `gensparx sandbox recreate --browser --all`.
...
```bash
gensparx security audit --json | jq '.summary'
gensparx security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
gensparx security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
