...
- Jobs persist under `~/.gensparx/cron/` so restarts don’t lose schedules.
...
gensparx cron list
gensparx cron run <job-id>
gensparx cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.gensparx/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `gensparx cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, gensparx applies a
...
If `delivery` is omitted for isolated jobs, gensparx defaults to `announce`.
...
- CLI equivalents: `gensparx cron add --light-context ...` and `gensparx cron edit --light-context`.
...
- Job store: `~/.gensparx/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.gensparx/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, gensparx classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, gensparx archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `gensparx cron runs` and adjust retention before logs become large
...
gensparx cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
gensparx cron edit <jobId> --agent ops
gensparx cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
gensparx cron run <jobId>
gensparx cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
gensparx cron edit <jobId> \
...
```bash
gensparx cron edit <jobId> --exact
```
...
```bash
gensparx system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`gensparx system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `GENSPARX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- gensparx applies exponential retry backoff for recurring jobs after consecutive errors:
