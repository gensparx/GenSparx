...
This document explains how gensparx manages sessions end-to-end:
...
gensparx is designed around a single **Gateway process** that owns session state.
...
gensparx persists sessions in two layers:
...
- Transcripts: `~/.gensparx/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

gensparx resolves these via `src/config/sessions.ts`.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.gensparx/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
gensparx intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (gensparx consumes the events, but Pi decides when to compact).
...
gensparx also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, gensparx bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, gensparx leaves it alone.
...
- `gensparx status` (CLI)
- `gensparx sessions` / `sessions --json`
...
gensparx supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- gensparx strips/suppresses this in the delivery layer.

As of `2026.1.10`, gensparx also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
gensparx uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Gensparx’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `gensparx status`.
