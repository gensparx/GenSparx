...
GenSparx integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- GenSparx owns lifecycle when `channels.signal.autoStart=true`.
...
- GenSparx spawns `imsg rpc` as a child process (legacy iMessage integration).
