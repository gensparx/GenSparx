...
- With Anthropic API-key auth profiles, gensparx seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, gensparx injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
gensparx exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.gensparx/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$GENSPARX_STATE_DIR/logs/cache-trace.jsonl`
...
- `GENSPARX_CACHE_TRACE=1` enables cache tracing.
- `GENSPARX_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `GENSPARX_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `GENSPARX_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `GENSPARX_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
