...
- Publish as `gensparx/plugin-sdk` (or export from core under `gensparx/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `GensparxPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: GensparxConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: GensparxConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: GensparxConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: GensparxConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: GensparxConfig): string;
  };
};
```
...
- Introduce `gensparx/plugin-sdk`.
- Add `api.runtime` to `GensparxPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `gensparxRuntime: ">=2026.2.0"`).
