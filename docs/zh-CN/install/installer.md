---
read_when:
  - 你想了解 `gensparx.com/install.sh` 的工作机制
  - 你想自动化安装（CI / 无头环境）
  - 你想从 GitHub 检出安装
...
gensparx 提供两个安装器脚本（托管在 `gensparx.com`）：

- `https://gensparx.com/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://gensparx.com/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://gensparx.com/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）
