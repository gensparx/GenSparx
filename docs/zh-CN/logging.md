...
你可以在 `~/.gensparx/gensparx.json` 中覆盖此设置：
...
所有日志配置都在 `~/.gensparx/gensparx.json` 的 `logging` 下。
...
- gensparx 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `gensparx plugins enable diagnostics-otel` 启用插件。
...
- `gensparx.tokens`（计数器，属性：`gensparx.token`、`gensparx.channel`、`gensparx.provider`、`gensparx.model`）
- `gensparx.cost.usd`（计数器，属性：`gensparx.channel`、`gensparx.provider`、`gensparx.model`）
- `gensparx.run.duration_ms`（直方图，属性：`gensparx.channel`、`gensparx.provider`、`gensparx.model`）
- `gensparx.context.tokens`（直方图，属性：`gensparx.context`、`gensparx.channel`、`gensparx.provider`、`gensparx.model`）

消息流：

- `gensparx.webhook.received`（计数器，属性：`gensparx.channel`、`gensparx.webhook`）
- `gensparx.webhook.error`（计数器，属性：`gensparx.channel`、`gensparx.webhook`）
- `gensparx.webhook.duration_ms`（直方图，属性：`gensparx.channel`、`gensparx.webhook`）
- `gensparx.message.queued`（计数器，属性：`gensparx.channel`、`gensparx.source`）
- `gensparx.message.processed`（计数器，属性：`gensparx.channel`、`gensparx.outcome`）
- `gensparx.message.duration_ms`（直方图，属性：`gensparx.channel`、`gensparx.outcome`）

队列 + 会话：

- `gensparx.queue.lane.enqueue`（计数器，属性：`gensparx.lane`）
- `gensparx.queue.lane.dequeue`（计数器，属性：`gensparx.lane`）
- `gensparx.queue.depth`（直方图，属性：`gensparx.lane` 或 `gensparx.channel=heartbeat`）
- `gensparx.queue.wait_ms`（直方图，属性：`gensparx.lane`）
- `gensparx.session.state`（计数器，属性：`gensparx.state`、`gensparx.reason`）
- `gensparx.session.stuck`（计数器，属性：`gensparx.state`）
- `gensparx.session.stuck_age_ms`（直方图，属性：`gensparx.state`）
- `gensparx.run.attempt`（计数器，属性：`gensparx.attempt`）
...
- `gensparx.webhook.error`
  - `gensparx.channel`、`gensparx.webhook`、`gensparx.chatId`、`gensparx.error`
- `gensparx.message.processed`
  - `gensparx.channel`、`gensparx.outcome`、`gensparx.chatId`、`gensparx.messageId`、`gensparx.sessionKey`、`gensparx.sessionId`、`gensparx.reason`
- `gensparx.session.stuck`
  - `gensparx.state`、`gensparx.ageMs`、`gensparx.queueDepth`、`gensparx.sessionKey`、`gensparx.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `gensparx doctor`。
