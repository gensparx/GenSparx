...
Note: replace `18789` with your configured `gateway.port` (or `--port`/`GENSPARX_GATEWAY_PORT`).
...
- Local mode defaults:
  - token: `GENSPARX_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token`
  - password: `GENSPARX_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password`
- Remote mode defaults:
  - token: `gateway.remote.token` -> `GENSPARX_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `GENSPARX_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Legacy `CLAWDBOT_GATEWAY_*` env vars are only used by compatibility call paths; probe/status/auth resolution uses `GENSPARX_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `GENSPARX_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
