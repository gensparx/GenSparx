...
```json5
{
  agent: { workspace: "~/.gensparx/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.gensparx/gensparx.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.gensparx/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
