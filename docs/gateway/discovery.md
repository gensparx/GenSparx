...
- Service types:
  - `_gensparx-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
...
- `GENSPARX_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.gensparx/gensparx.json` controls the Gateway bind mode.
- `GENSPARX_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `GENSPARX_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `GENSPARX_CLI_PATH` overrides the advertised CLI path.
