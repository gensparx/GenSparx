...
read_when:
  - Running gensparx behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Gensparx
...
- You run gensparx behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. gensparx checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. gensparx extracts the user identity from the configured header
...
- gensparx can stay on loopback HTTP behind the proxy.
...
If gensparx itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://gensparx.example.com
    to: http://gensparx-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
gensparx.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy gensparx:18789 {
...
location / {
    ...
    proxy_pass http://gensparx:18789;
...
`gensparx security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update gensparx config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `gensparx security audit` and review findings
