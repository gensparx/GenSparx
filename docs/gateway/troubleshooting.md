...
- `gensparx gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `gensparx doctor` reports no blocking config/service issues.
- `gensparx channels status --probe` shows connected/ready channels.
...
gensparx config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `gensparx configure`). If you are running gensparx via Podman using the dedicated `gensparx` user, the config lives at `~gensparx/.gensparx/gensparx.json`.
...
gensparx system heartbeat last
gensparx logs --follow
```
...
gensparx approvals get --node <idOrNameOrIp>
gensparx logs --follow
gensparx status
```
...
gensparx browser start --browser-profile gensparx
...
gensparx config get gateway.auth.token
gensparx gateway status
gensparx logs --follow
```
