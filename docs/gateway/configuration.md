...
GenSparx reads an optional **JSON5** config from `~/.openclaw/openclaw.json` (comments + trailing commas allowed).

If the file is missing, GenSparx uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.openclaw/workspace`). You usually only need a config to:
...
GenSparx only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `gensparx doctor`, `gensparx logs`, `gensparx health`, `gensparx status`, `gensparx service`, `gensparx help`).
- Run `gensparx doctor` to see the exact issues.
- Run `gensparx doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `gensparx config set`. Keep a backup of `~/.openclaw/openclaw.json`.
...
<Warning>
GenSparx only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
{ "$include": "/etc/GenSparx/base.json5" } // absolute
...
GenSparx reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, GenSparx runs your login shell and imports only the missing expected keys (never overrides).
...
GenSparx stores **per-agent** auth profiles (OAuth + API keys) in:
...
- `~/.openclaw/agent/*` (migrated by `gensparx doctor` into `~/.openclaw/agents/<defaultAgentId>/agent/*`)
...
On first use, GenSparx imports `oauth.json` entries into `auth-profiles.json`.
...
If set, GenSparx derives defaults (only when you haven’t set them explicitly):
...
- If you want a stable path, set `logging.file` to `/tmp/GenSparx/GenSparx.log`.
...
- The legacy single-account Baileys auth dir is migrated by `gensparx doctor` into `whatsapp/default`.
...
GenSparx starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["GenSparx-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
GenSparx starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `gensparx plugins install @GenSparx/mattermost` (or `./extensions/mattermost` from a git checkout).
...
GenSparx starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Optional repository root to show in the system prompt’s Runtime line. If unset, GenSparx
...
When a file exceeds this limit, GenSparx logs a warning and injects a truncated
...
message envelopes). If unset, GenSparx uses the host timezone at runtime.
...
`[gensparx]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[gensparx]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[gensparx]"`, GenSparx will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, GenSparx generates audio
...
GenSparx also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, GenSparx currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `gensparx configure --section web`, or use `BRAVE_API_KEY` env var)
...
GenSparx uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.openclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
GenSparx config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, GenSparx writes/merges a `models.json` into
...
OpenCode Zen is a multi-model gateway with per-model endpoints. GenSparx uses
...
- Shortcut: `gensparx onboard --auth-choice opencode-zen`.
...
Shortcut: `gensparx onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `gensparx onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `gensparx onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `gensparx onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `gensparx onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `gensparx onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, GenSparx stays in idle-only mode for backward compatibility.
...
from `~/.openclaw/extensions`, `<workspace>/.GenSparx/extensions`, plus any
...
### `browser` (GenSparx-managed browser)

GenSparx can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for gensparx and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (GenSparx.app menubar, or `gensparx gateway`).
...
- Examples: `"/ui"`, `"/GenSparx"`, `"/apps/GenSparx"`.
...
- When a connection comes from one of these IPs, GenSparx uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `gensparx gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. GenSparx
...
- GenSparx.app watches `~/.openclaw/openclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
- `gensparx --dev …` → uses `~/.openclaw-dev` + shifts ports from base `19001`
- `gensparx --profile <name> …` → uses `~/.openclaw-<name>` (port via config/env/flags)
...
Gmail helper config (used by `gensparx webhooks gmail setup` / `run`):
...
Note: when `tailscale.mode` is on, GenSparx defaults `serve.path` to `/` so
...
Default port: `18793` (chosen to avoid the gensparx browser CDP port `18792`)  
...
- Hostname: defaults to `gensparx` (advertises `GenSparx.local`). Override with `OPENCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.openclaw/dns/` using the configured discovery domain (example: `GenSparx.internal.`).
