...
- During heartbeat runs, GenSparx treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `gensparx sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, GenSparx skips the heartbeat run entirely (no model call).
...
```bash
gensparx system event --text "Check for urgent follow-ups" --mode now
```
