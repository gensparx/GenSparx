...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `GENSPARX_GATEWAY_TOKEN` (or legacy token env vars) is set, because env token input wins for that runtime.
