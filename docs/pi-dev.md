...
```bash
GENSPARX_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
State lives under the gensparx state directory. Default is `~/.gensparx`. If `GENSPARX_STATE_DIR` is set, use that directory instead.

To reset everything:

- `gensparx.json` for config
