---
summary: "gensparx on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up gensparx on a Raspberry Pi
  - Running gensparx on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# gensparx on Raspberry Pi

## Goal

Run a persistent, always-on gensparx Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
gensparx config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/gensparx-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/gensparx-compile-cache
mkdir -p /var/tmp/gensparx-compile-cache
export GENSPARX_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `GENSPARX_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Gensparx, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit gensparx
```

```ini
[Service]
Environment=GENSPARX_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/gensparx-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep gensparx state/cache on SSD-backed storage to avoid SD-card
...
Most gensparx features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled gensparx

# Enable if not
sudo systemctl enable gensparx

# Start on boot
sudo systemctl start gensparx
```
...
```bash
# Check logs
journalctl -u gensparx --no-pager -n 100

# Common fix: rebuild
cd ~/gensparx  # if using hackable install
npm run build
sudo systemctl restart gensparx
```
