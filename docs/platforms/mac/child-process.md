...
service via the external `gensparx` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.gensparx.gateway`
  (or `ai.gensparx.<profile>` when using `--profile`/`GENSPARX_PROFILE`; legacy `com.gensparx.*` is supported).
