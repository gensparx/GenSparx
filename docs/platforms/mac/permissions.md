...
```bash
sudo tccutil reset Accessibility ai.gensparx.mac
sudo tccutil reset ScreenCapture ai.gensparx.mac
...
Workaround: move files into the gensparx workspace (`~/.gensparx/workspace`) if you want to avoid per-folder grants.
