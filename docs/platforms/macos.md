...
The app manages a per‑user LaunchAgent labeled `ai.gensparx.gateway`
(or `ai.gensparx.<profile>` when using `--profile`/`GENSPARX_PROFILE`; legacy `com.gensparx.*` still unloads).

```bash
launchctl kickstart -k gui/$UID/ai.gensparx.gateway
launchctl bootout gui/$UID/ai.gensparx.gateway
```

Replace the label with `ai.gensparx.<profile>` when running a named profile.
...
Avoid putting your gensparx state dir in iCloud or other cloud-synced folders.
...
```bash
GENSPARX_STATE_DIR=~/.gensparx
```

If `gensparx doctor` detects state under:
