...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [I can't access docs.gensparx.com (SSL error). What now?](#i-cant-access-docsgensparxcom-ssl-error-what-now)
...
https://github.com/GenSparx/GenSparx/issues
https://github.com/GenSparx/GenSparx/pulls
...
- If it asks for auth, run `gensparx dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `OPENCLAW_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `gensparx gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
...
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from `gensparx dashboard`.
...
https://github.com/GenSparx/GenSparx/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.gensparx.com SSL error What now
...
detail: [Troubleshooting](/help/troubleshooting#docsgensparxcom-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: https://spa.xfinity.com/check_url_status.

If you still can't reach the site, the docs are mirrored on GitHub:
https://github.com/GenSparx/GenSparx/tree/main/docs
...
https://gensparx.com/install.ps1
...
GenSparx supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. GenSparx fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Usually no. GenSparx needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
Docs: [BlueBubbles](/channels/bluebubbles), [Nodes](/nodes), [Mac remote mode](/platforms/macos).
...
[Mac remote mode](/platforms/macos).
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.openclaw/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which GenSparx treats as `<workspace>/skills`.
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured GenSparx workspace); GenSparx treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
configure it. If you have a local model path configured and present, GenSparx
prefers `local`.
...
Legacy single-agent path: `~/.openclaw/agent/*` (migrated by `gensparx doctor`).
...
GenSparx reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
Docs: [Nodes](/nodes), [Gateway protocol](/gateway/protocol), [macOS remote mode](/platforms/macos), [Security](/gateway/security).
...
Docs: [Gateway protocol](/gateway/protocol), [Discovery](/gateway/discovery), [macOS remote mode](/platforms/macos).
...
### Why doesnt GenSparx reply in a group
...
Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, GenSparx currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `gensparx configure --section models` (interactive)
...
- **GenSparx + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.openclaw/agent/*` (migrated by `gensparx doctor`)
...
- Fastest: `gensparx dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
...
- Use `gensparx --profile <name> …` per instance (auto-creates `~/.openclaw-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `gensparx --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.GenSparx.*`, `GenSparx-gateway-<profile>.service`, `GenSparx Gateway (<profile>)`).
...
- Default behavior on DM-capable channels is **pairing**:
  ...
  - Approve with: `gensparx pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `gensparx pairing list <channel>` if a code didn't arrive.
