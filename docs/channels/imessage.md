...
```
  ...
  <Tab title="Remote Mac over SSH">
    gensparx only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```
    ...
    If `remoteHost` is not set, gensparx attempts to auto-detect it by parsing the SSH wrapper script.
    `remoteHost` must be `host` or `user@host` (no spaces or SSH options).
    gensparx uses strict host-key checking for SCP, so the relay host key must already exist in `~/.ssh/known_hosts`.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`gensparx pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs Gensparx/`imsg`.
