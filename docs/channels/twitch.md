...
read_when:
  - Setting up Twitch chat integration for Gensparx
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. gensparx connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
gensparx plugins install @gensparx/twitch
```

Local checkout (when running from a git repo):

```bash
gensparx plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `GENSPARX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "gensparx", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use GENSPARX_TWITCH_ACCESS_TOKEN env var)
...
```bash
GENSPARX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
