...
```bash
gensparx config set channels.discord.token '"YOUR_BOT_TOKEN"' --json
gensparx config set channels.discord.enabled true --json
gensparx gateway
```

    If gensparx is already running as a background service, use `gensparx gateway restart` instead.
...
```bash
gensparx pairing list discord
gensparx pairing approve discord <CODE>
```
...
- Use `gensparx message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
gensparx message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
gensparx message thread create --channel discord --target channel:<forumId> \
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `gensparx security audit` warns when name/tag entries are used
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/gensparx",
    },
  },
}
```
...
- gensparx also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
- Any audio format is accepted; gensparx converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep gensparx current (`gensparx update`) so the Discord voice receive recovery logic is present
...
- If command deploy/state is stale, restart gateway and re-check with `gensparx channels status --probe`.
