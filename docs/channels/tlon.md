...
Tlon is a decentralized messenger built on Urbit. GenSparx connects to your Urbit ship and can
...
```bash
gensparx plugins install @openclaw/tlon
```

Local checkout (when running from a git repo):

```bash
gensparx plugins install ./extensions/tlon
```
...
By default, GenSparx blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `gensparx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, GenSparx replies in-thread.
