...
```bash
gensparx plugins install @gensparx/msteams
```

Local checkout (when running from a git repo):

```bash
gensparx plugins install ./extensions/msteams
```
...
3. Configure gensparx with those credentials.
...
- Talk to gensparx via Teams DMs, group chats, or channels.
...
- On startup, gensparx resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.gensparx/gensparx.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Gensparx, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `gensparx-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `gensparx plugins install @gensparx/msteams`
   - From a local checkout: `gensparx plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "gensparx in Teams", "full": "gensparx in Teams" },
...
gensparx handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, gensparx only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/GensparxShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

gensparx sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `gensparx message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.gensparx/msteams-polls.json`.
...
```bash
gensparx message send --channel msteams \
...
gensparx message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
gensparx message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
gensparx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
gensparx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
