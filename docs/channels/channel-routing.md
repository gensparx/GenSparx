...
gensparx infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, gensparx still records inbound session metadata, but it
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.gensparx/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.gensparx`):

- `~/.gensparx/agents/<agentId>/sessions/sessions.json`
