...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Gensparx"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure gensparx and restart the gateway.
5. Send a first DM and approve pairing (`gensparx pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Gensparx"` then scan the QR in Signal.
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `gensparx pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point gensparx at it:
...
This skips auto-spawn and the startup wait inside Gensparx. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `gensparx pairing list signal`
  - `gensparx pairing approve signal <CODE>`
...
- **Typing indicators**: gensparx sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, gensparx forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `gensparx doctor --fix`.
...
gensparx pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/gensparx/gensparx-$(date +%Y-%m-%d).log" | tail -20
```
