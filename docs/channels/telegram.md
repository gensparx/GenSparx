...
```
    ...
    Telegram does **not** use `gensparx channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `gensparx doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `gensparx doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `gensparx logs --follow`
...
- DM messages can carry `message_thread_id`; gensparx routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    gensparx can stream partial replies in real time:
    ...
    - DM: gensparx updates the draft in place (no extra preview message)
    - group/topic: gensparx keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), gensparx falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, gensparx skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, gensparx automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, gensparx retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - gensparx pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.gensparx/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, gensparx enqueues system events like:
...
```bash
gensparx message send --channel telegram --target 123456789 --message "hi"
gensparx message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `gensparx message poll` and support forum topics:

```bash
gensparx message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
gensparx message poll --channel telegram --target -1001234567890:topic:42 \
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `gensparx channels status` warns when config expects unmentioned group messages.
    - `gensparx channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `gensparx logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, gensparx now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `GENSPARX_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `GENSPARX_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `GENSPARX_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `gensparx doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `gensparx doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, gensparx falls back to the first normalized account ID and `gensparx doctor` warns.
