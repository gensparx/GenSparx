...
read_when:
  - Setting up Zalo Personal for Gensparx
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Gensparx.
...
- Install via CLI: `gensparx plugins install @gensparx/zalouser`
- Or from a source checkout: `gensparx plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `gensparx channels login --channel zalouser`
...
- `gensparx pairing list zalouser`
- `gensparx pairing approve zalouser <code>`
...
- On startup, gensparx resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in gensparx state. Example:
...
- gensparx sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, gensparx sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `gensparx channels logout --channel zalouser && gensparx channels login --channel zalouser`
...
- The channel now runs fully in gensparx without external CLI binaries.
