---
summary: "Run gensparx with Ollama (local LLM runtime)"
read_when:
  - You want to run gensparx with local models via Ollama
...
Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. gensparx integrates with Ollama's native API (`/api/chat`), supporting streaming and tool calling, and can **auto-discover tool-capable models** when you opt in with `OLLAMA_API_KEY` (or an auth profile) and do not define an explicit `models.providers.ollama` entry.

<Warning>
**Remote Ollama users**: Do not use the `/v1` OpenAI-compatible URL (`http://host:11434/v1`) with Gensparx. This breaks tool calling and models may output raw tool JSON as plain text. Use the native Ollama API URL instead: `baseUrl: "http://host:11434"` (no `/v1`).
</Warning>
...
3. Enable Ollama for gensparx (any value works; Ollama doesn't require a real key):
...
gensparx config set models.providers.ollama.apiKey "ollama-local"
```
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, gensparx discovers models from the local Ollama instance at `http://127.0.0.1:11434`:
...
If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and gensparx will fill it for availability checks.
...
gensparx marks models as reasoning-capable when Ollama reports `thinking` in `/api/show`:
...
For auto-discovered models, gensparx uses the context window reported by Ollama when available, otherwise it defaults to `8192`. You can override `contextWindow` and `maxTokens` in explicit provider config.
...
gensparx only auto-discovers models that report tool support. If your model isn't listed, either:
