---
summary: "Use Venice AI privacy-focused models in Gensparx"
read_when:
  - You want privacy-focused inference in Gensparx
...
```bash
gensparx onboard --auth-choice venice-api-key
```
...
```bash
gensparx agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, gensparx shows all available Venice models. Pick based on your needs:
...
```bash
gensparx models set venice/claude-opus-45
gensparx models set venice/llama-3.3-70b
```

List all available models:

```bash
gensparx models list | grep venice
```

## Configure via `gensparx configure`

1. Run `gensparx configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
gensparx automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
gensparx agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
gensparx agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
gensparx agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
gensparx agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
gensparx agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `gensparx models list` to see currently available models. Some models may be temporarily offline.
