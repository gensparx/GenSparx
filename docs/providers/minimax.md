---
summary: "Use MiniMax M2.5 in Gensparx"
read_when:
  - You want MiniMax models in Gensparx
...
- **Compatibility:** gensparx still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
gensparx plugins enable minimax-portal-auth  # skip if already loaded.
gensparx gateway restart  # restart if gateway is already running
gensparx onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/gensparx/gensparx/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `gensparx.json`:
...
- Use `gensparx models list` and `gensparx models set minimax/MiniMax-M2.5` to switch.
...
- Running `gensparx configure` and selecting **MiniMax M2.5**, or
