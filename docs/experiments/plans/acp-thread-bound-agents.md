...
This plan defines how GenSparx should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- GenSparx core owns ACP control plane concerns
...
GenSparx should not reimplement ACP transport internals in core.
GenSparx should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in GenSparx, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under GenSparx state dir
...
- ACP event stream is normalized to GenSparx reply chunks
...
For the first production backend (`extensions/acpx`), GenSparx and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses GenSparx `sessionKey`)
...
- GenSparx consumes ndjson events from `acpx --format json --json-strict`
