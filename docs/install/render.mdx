...
Deploy gensparx on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/gensparx/gensparx)
...
repository configures everything needed to run Gensparx:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: GENSPARX_STATE_DIR
        value: /data/.gensparx
      - key: GENSPARX_WORKSPACE_DIR
        value: /data/workspace
      - key: GENSPARX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/gensparx`.
...
If you use the original gensparx repository, Render will not auto-deploy your Gensparx. To update it, run a manual Blueprint sync from the dashboard.
...
For Gensparx, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any gensparx host.
