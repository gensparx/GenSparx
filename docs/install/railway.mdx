...
Deploy gensparx on Railway with a one-click template and finish setup in your browser.
...
<Note>
  Railway still uses the legacy template slug `clawdbot-railway-template`. The deployed service and
  setup flow are Gensparx.
</Note>
...
- `https://<your-railway-domain>/gensparx` — Control UI

## What you get

- Hosted gensparx Gateway + Control UI
...
- `GENSPARX_STATE_DIR=/data/.gensparx` (recommended)
- `GENSPARX_WORKSPACE_DIR=/data/workspace` (recommended)
- `GENSPARX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your gensparx state + workspace so you can migrate to another host without losing config or memory.
