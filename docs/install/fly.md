...
summary: "Step-by-step Fly.io deployment for gensparx with persistent storage and HTTPS"
read_when:
  - Deploying gensparx on Fly.io
...
[env]
  NODE_ENV = "production"
  GENSPARX_PREFER_PNPM = "1"
  GENSPARX_STATE_DIR = "/data"
...
| `internal_port = 3000`         | Must match `--port 3000` (or `GENSPARX_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `GENSPARX_STATE_DIR = "/data"` | Persists state on the volume                                                |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set GENSPARX_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `GENSPARX_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `gensparx.json` where they could be accidentally exposed or logged.
...
**Note:** With `GENSPARX_STATE_DIR=/data`, the config path is `/data/gensparx.json`.
...
Paste your gateway token (the one from `GENSPARX_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `GENSPARX_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/gensparx.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/gensparx.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/gensparx.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/gensparx.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/gensparx.json"
```
...
**Fix:** Ensure `GENSPARX_STATE_DIR=/data` is set in `fly.toml` and redeploy.
