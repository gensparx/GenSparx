...
GenSparx ships three update channels:
...
When you **explicitly** switch channels with `--channel`, GenSparx also aligns
the install method:

- `dev` ensures a git checkout (default `~/GenSparx`, override with `OPENCLAW_GIT_DIR`),
...
When you switch channels with `gensparx update`, GenSparx also syncs plugin sources:
