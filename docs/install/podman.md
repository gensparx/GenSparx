...
**3. Onboarding wizard** (for example, to add channels or providers):
...
## The service user (non-login)

`setup-podman.sh` creates a dedicated system user `openclaw`:

- **Shell:** `nologin` - no interactive login; reduces attack surface.
- **Home:** for example `/home/openclaw` - holds `~/.openclaw` (config, workspace) and the launch script `run-openclaw-podman.sh`.
- **Rootless Podman:** The user must have a **subuid** and **subgid** range. Many distros assign these automatically when the user is created. If setup prints a warning, add lines to `/etc/subuid` and `/etc/subgid`:
  ...
  Then start the gateway as that user (for example, from cron or systemd):
...
- **Optional:** In that `.env` you can set provider keys (for example `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other GenSparx env vars.
...
- **Rootless Podman fails for user openclaw:** Check `/etc/subuid` and `/etc/subgid` contain a line for `openclaw` (for example `openclaw:100000:65536`). Add it if missing and restart.
...
- **Script not found when running as openclaw:** Ensure `setup-podman.sh` was run so that `run-openclaw-podman.sh` is copied to openclaw's home (for example `/home/openclaw/run-openclaw-podman.sh`).
