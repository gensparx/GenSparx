...
- builds the gateway image locally (or pulls a remote image if `GENSPARX_IMAGE` is set)
...
- `GENSPARX_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/gensparx/gensparx:latest`)
- `GENSPARX_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `GENSPARX_EXTRA_MOUNTS` — add extra host bind mounts
- `GENSPARX_HOME_VOLUME` — persist `/home/node` in a named volume
- `GENSPARX_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `GENSPARX_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `GENSPARX_SANDBOX=1` for local builds.
- `GENSPARX_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `GENSPARX_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `GENSPARX_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `GENSPARX_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `GENSPARX_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm gensparx-cli dashboard --no-open`.
...
```bash
export GENSPARX_SANDBOX=1
export GENSPARX_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `gensparx-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `GENSPARX_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm gensparx-cli gateway probe
docker compose run -T --rm gensparx-cli devices list --json
```
...
`gensparx-cli` uses `network_mode: "service:gensparx-gateway"` so CLI commands can
...
network path instead of the bundled `gensparx-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `gensparx-cli`.
...
- [GitHub Container Registry package](https://github.com/gensparx/gensparx/pkgs/container/gensparx)

Use image name `ghcr.io/gensparx/gensparx` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/gensparx/gensparx`
- `org.opencontainers.image.url=https://gensparx.com`
- `org.opencontainers.image.documentation=https://docs.gensparx.com/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=Gensparx`
- `org.opencontainers.image.description=gensparx gateway and CLI runtime container image`
...
image instead, set `GENSPARX_IMAGE` before running the script:

```bash
export GENSPARX_IMAGE="ghcr.io/gensparx/gensparx:latest"
./docker-setup.sh
```

The script detects that `GENSPARX_IMAGE` is not the default `gensparx:local` and
...
`GENSPARX_EXTRA_MOUNTS` or `GENSPARX_HOME_VOLUME`, the setup script writes
...
`GENSPARX_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
...
```bash
export GENSPARX_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `GENSPARX_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `GENSPARX_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export GENSPARX_HOME_VOLUME="gensparx_home"
./docker-setup.sh
```
...
- If you change `GENSPARX_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `GENSPARX_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export GENSPARX_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `GENSPARX_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export GENSPARX_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`GENSPARX_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `GENSPARX_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `GENSPARX_EXTRA_MOUNTS`.
...
background. In plain terms: Docker keeps checking if gensparx is still
...
```bash
docker compose exec gensparx-gateway node dist/index.js health --token "$GENSPARX_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `GENSPARX_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm gensparx-cli config set gateway.mode local
docker compose run --rm gensparx-cli config set gateway.bind lan
docker compose run --rm gensparx-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`GENSPARX_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.gensparx/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.gensparx/sandboxes`
...
- Browser containers default to a dedicated Docker network (`gensparx-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; gensparx provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `GENSPARX_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `GENSPARX_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `GENSPARX_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
