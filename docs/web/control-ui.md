...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/gensparx`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop gensparx`, `please stop`) to abort out-of-band
