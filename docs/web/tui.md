...
```bash
gensparx tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `gensparx tui --deliver`
...
- Local shell commands receive `GENSPARX_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `gensparx logs --follow`.
- Confirm the agent can run: `gensparx status` and `gensparx models status`.
...
- No agents in picker: check `gensparx agents list` and your routing config.
