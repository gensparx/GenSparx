...
- **Localhost**: open `http://127.0.0.1:18789/`. If you see “unauthorized,” run `gensparx dashboard` and use the tokenized link (`?token=...`).
...
- Run `gensparx dashboard` to get a fresh tokenized link.
- Ensure the gateway is reachable (local: `gensparx status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`).
