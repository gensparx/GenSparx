...
- Documentation links updated: `docs.gensparx.com` → `docs.gensparx.com`
...
- CLI: add `openclaw system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.gensparx.com/cli/system
- CLI: add live auth probes to `openclaw models status` for per-profile verification. (commit 40181afde) https://docs.gensparx.com/cli/models
...
- CLI: add `openclaw update wizard` for interactive channel selection and restart prompts. https://docs.gensparx.com/cli/update
...
- ACP: add `openclaw acp` for IDE integrations. https://docs.gensparx.com/cli/acp
- ACP: add `openclaw acp client` interactive harness for debugging. https://docs.gensparx.com/cli/acp
...
- Exec approvals: migrate approvals to `~/.openclaw/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.gensparx.com/tools/exec-approvals
- Nodes: add headless node host (`openclaw node start`) for `system.run`/`system.which`. https://docs.gensparx.com/cli/node
...
- Plugins: add Zalo Personal plugin (`@openclaw/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.gensparx.com/plugins/zalouser
...
- **BREAKING:** `openclaw hooks` is now `openclaw webhooks`; hooks live under `openclaw hooks`. https://docs.gensparx.com/cli/webhooks
