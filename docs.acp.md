...
This document describes how the gensparx ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`gensparx acp` exposes an ACP agent over stdio and forwards prompts to a running
gensparx Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `gensparx acp` and speaks ACP messages over stdio.
...
`gensparx acp` resolves the Gateway URL and auth from CLI flags or config:
