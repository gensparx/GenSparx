...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: GensparxProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: GensparxProtocol.AnyCodable] = [
            "presence": .init([GensparxProtocol.AnyCodable(entry)]),
        ]
        let payload = GensparxProtocol.AnyCodable(payloadMap)
