...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersGensparxBinary() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let gensparxPath = tmp.appendingPathComponent("node_modules/.bin/gensparx")
        try makeExecutableForTests(at: gensparxPath)

        let cmd = CommandResolver.gensparxCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([gensparxPath.path, "gateway"]))
    }
    ...
    @Test func prefersGensparxBinaryOverPnpm() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let gensparxPath = binDir.appendingPathComponent("gensparx")
        ...
        #expect(cmd.prefix(2).elementsEqual([gensparxPath.path, "rpc"]))
    }

    @Test func usesGensparxBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "gensparx", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "gensparx", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("gensparx@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/gensparx", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "gensparx@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/gensparx'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("gensparx"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == gensparxPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
