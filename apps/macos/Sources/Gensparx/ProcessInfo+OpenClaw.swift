...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.gensparx.mac`) even if the shipped
    ...
    {
        if environment["GENSPARX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "gensparx.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "gensparx.nixMode") { return true }
