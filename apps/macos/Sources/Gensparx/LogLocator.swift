...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["GENSPARX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/gensparx")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("gensparx-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("gensparx-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/gensparx/ (rolling or stdout), or nil if none exist.
