...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.gensparx", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> GensparxSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: GensparxSessionPreviewEntry,
    ...
    private static func cache(payload: GensparxSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: GensparxChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [GensparxKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [GensparxKit.AnyCodable]) -> [GensparxChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(GensparxChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: GensparxChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: GensparxChatMessage) -> Bool {
    ...
    private static func toolNames(for message: GensparxChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: GensparxChatMessage) -> String? {
