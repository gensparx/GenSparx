...
enum CommandResolver {
    private static let projectRootDefaultsKey = "gensparx.gatewayProjectRootPath"
    private static let helperName = "gensparx"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let gensparxEntry = root.appendingPathComponent("gensparx.mjs").path
        if FileManager().isReadableFile(atPath: gensparxEntry) { return gensparxEntry }
        let binEntry = root.appendingPathComponent("bin/gensparx.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/gensparx")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let gensparxPaths = self.gensparxManagedPaths(home: home)
        if !gensparxPaths.isEmpty {
            extras.insert(contentsOf: gensparxPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + gensparxPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func gensparxManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".gensparx"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func gensparxExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectGensparxExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("gensparx.mjs").path,
            root.appendingPathComponent("bin/gensparx.js").path,
        ]
    ...
    static func hasAnyGensparxInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.gensparxExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func gensparxNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let gensparxPath = self.projectGensparxExecutable(projectRoot: root) {
            return [gensparxPath, subcommand] + extraArgs
        }
        if let gensparxPath = self.gensparxExecutable(searchPaths: searchPaths) {
            return [gensparxPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "gensparx", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            gensparx entrypoint missing (looked for dist/index.js or gensparx.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real gensparx CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/gensparx"
        ...
        \(cliSection)
        \(projectSection)
        if command -v gensparx >/dev/null 2>&1; then
          CLI="$(command -v gensparx)"
          gensparx \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/gensparx.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/gensparx.mjs"
            node "$PRJ/gensparx.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/gensparx.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/gensparx.js"
            node "$PRJ/bin/gensparx.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent gensparx"
          pnpm --silent gensparx \(quotedArgs);
        else
          echo "gensparx CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? GensparxConfigFile.loadDict()
