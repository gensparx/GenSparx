...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.gensparx", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.gensparx", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [GensparxChatMessage] = messages.compactMap { item in
