...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.gensparx/gensparx.json. Prefer GENSPARX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.gensparx/gensparx.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.gensparx/gensparx.json. Restart the gateway to apply."
        }
