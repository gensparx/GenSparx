...
struct GensparxMacCLI {
    static func main() async {
        ...
        default:
            fputs("gensparx-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      gensparx-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      gensparx-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      gensparx-mac connect
      gensparx-mac connect --url ws://127.0.0.1:18789 --json
      gensparx-mac discover --timeout 3000 --json
      gensparx-mac wizard --mode local
    """)
}
