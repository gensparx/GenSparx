...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.gensparx", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = GensparxConfigFile.hostKey(sshHost)
        let urlKey = GensparxConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.gensparx.remote.tunnel.port", qos: .utility)
            do {
