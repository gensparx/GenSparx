...
enum DebugActions {
    private static let verboseDefaultsKey = "gensparx.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "gensparx.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = GensparxPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Gensparx", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = GensparxPaths.configURL
        guard
