...
private let webChatSwiftLogger = Logger(subsystem: "ai.gensparx", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: GensparxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> GensparxChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(GensparxChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> GensparxChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                GensparxGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    GensparxAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<GensparxChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any GensparxChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = GensparxChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: GensparxChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<GensparxChatView>) -> NSViewController
    {
