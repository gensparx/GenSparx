...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> GensparxDeviceStatusPayload {
        ...
        return GensparxDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> GensparxDeviceInfoPayload {
        ...
        return GensparxDeviceInfoPayload(
    ...
    private func batteryStatus() -> GensparxBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: GensparxBatteryState = switch device.batteryState {
        ...
        return GensparxBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> GensparxThermalStatusPayload {
        let state: GensparxThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return GensparxThermalStatusPayload(state: state)
    }

    private func storageStatus() -> GensparxStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return GensparxStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
