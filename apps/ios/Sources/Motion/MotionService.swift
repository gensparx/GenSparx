...
final class MotionService: MotionServicing {
    func activities(params: GensparxMotionActivityParams) async throws -> GensparxMotionActivityPayload {
        ...
        let mapped: [GensparxMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return GensparxMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: GensparxPedometerParams) async throws -> GensparxPedometerPayload {
        ...
        let payload: GensparxPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = GensparxPedometerPayload(
