...
struct IOSGatewayChatTransport: GensparxChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.gensparx", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> GensparxChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(GensparxChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> GensparxChatHistoryPayload {
        ...
        return try JSONDecoder().decode(GensparxChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [GensparxChatAttachmentPayload]) async throws -> GensparxChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [GensparxChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(GensparxChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(GensparxGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GensparxGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GensparxChatEventPayload.self)
                        {
