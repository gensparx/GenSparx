...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__GENSPARX_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<GensparxTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.gensparx", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = GensparxTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> GensparxTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return GensparxTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> GensparxTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> GensparxTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return GensparxTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> GensparxTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = GensparxTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: GensparxTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: GensparxAgentEventPayload.self
            ) else {
                continue
            }
