...
final class ContactsService: ContactsServicing {
    ...
    func search(params: GensparxContactsSearchParams) async throws -> GensparxContactsSearchPayload {
        ...
        return GensparxContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: GensparxContactsAddParams) async throws -> GensparxContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return GensparxContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return GensparxContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> GensparxContactPayload {
        GensparxContactPayload(
