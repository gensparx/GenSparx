...
final class RemindersService: RemindersServicing {
    func list(params: GensparxRemindersListParams) async throws -> GensparxRemindersListPayload {
        ...
        let payload: [GensparxReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return GensparxReminderPayload(
        ...
        return GensparxRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: GensparxRemindersAddParams) async throws -> GensparxRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = GensparxReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return GensparxRemindersAddPayload(reminder: payload)
    }
