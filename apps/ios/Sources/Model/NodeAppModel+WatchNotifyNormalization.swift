...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: GensparxWatchNotifyParams) -> GensparxWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [GensparxWatchAction]?,
        kind: String?,
        promptId: String?) -> [GensparxWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> GensparxWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return GensparxWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                GensparxWatchAction(id: "approve", label: "Approve"),
                GensparxWatchAction(id: "decline", label: "Decline", style: "destructive"),
                GensparxWatchAction(id: "open_phone", label: "Open iPhone"),
                GensparxWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            GensparxWatchAction(id: "done", label: "Done"),
            GensparxWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            GensparxWatchAction(id: "open_phone", label: "Open iPhone"),
            GensparxWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: GensparxWatchRisk?,
        priority: GensparxNotificationPriority?) -> GensparxWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: GensparxNotificationPriority?,
        risk: GensparxWatchRisk?) -> GensparxNotificationPriority?
    {
