...
private const val NOTIFICATION_CHANNEL_BASE_ID = "gensparx.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "GenSparx Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "GenSparx Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "GenSparx Active")
      }
