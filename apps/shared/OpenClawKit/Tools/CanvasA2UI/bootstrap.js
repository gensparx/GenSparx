...
import "@a2ui/lit/ui";
import { themeContext } from "@gensparx/a2ui-theme-context";
...
class GensparxA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    for (const eventName of ["gensparx:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.gensparxCanvasA2UIAction ??
      globalThis.gensparxCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.gensparxCanvasA2UIAction) {
...
if (!customElements.get("gensparx-a2ui-host")) {
  customElements.define("gensparx-a2ui-host", GensparxA2UIHost);
}
