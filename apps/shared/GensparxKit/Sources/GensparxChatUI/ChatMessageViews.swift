...
struct ChatMessageBubble: View {
    let message: GensparxChatMessage
    let style: GensparxChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? GensparxChatTheme.userText : GensparxChatTheme.assistantText
    ...
    private var inlineAttachments: [GensparxChatMessageContent] {
    ...
    private var toolCalls: [GensparxChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [GensparxChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? GensparxChatTheme.userBubble
        }
        if self.style == .onboarding {
            return GensparxChatTheme.onboardingAssistantBubble
        }
        return GensparxChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return GensparxChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: GensparxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? GensparxChatTheme.userText : GensparxChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: GensparxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(GensparxChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(GensparxChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [GensparxChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: GensparxChatTheme.assistantText)
            }
        }
    }
}
